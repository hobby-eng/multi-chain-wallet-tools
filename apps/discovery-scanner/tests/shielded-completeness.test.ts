  ...
  ledger.applyPage(0n, page, [
    {
      ...
      incoming: {
        ...
        memoHex: '',
        memo: '',
        noteNullifier: '11'.repeat(32),
      },
    },
  ]);
  const section = (complete: boolean) =>
    sectionFromLedger(
      ledger,
      { includeUsedZeroBalance: true, accountPathLabel: 'FVK' },
      { complete, pageCount: 1, terminalPosition: 1n },
      false,
      () => {},
    );
...
it('does not infer a zero account balance from an empty incoming-only scan', () => {
  const section = sectionFromLedger(
    new ShieldedActivityLedger('incoming'),
    { includeUsedZeroBalance: true, accountPathLabel: 'IVK' },
    { complete: true, pageCount: 2, terminalPosition: 0n },
    false,
    () => {},
  );
  expect(section.state).toBe('complete');
  expect(section.balanceAvailable).toBe(false);
  expect(summarizeDashSections([section]).find(({ label }) => label === 'Shielded spendable')!.value).toBe(
    'Unavailable',
  );
});
