...
const range = {
  ...
  customPathRangeEnd: "m/44'/5'/8'/0/{index}",
  customPathCount: 1,
};
...
describe('custom account range and free-form editor', () => {
  it('keeps arbitrary single paths and ignores an inactive custom range', () => {
    const paths = [
      ...customScanPaths({ scanCustomPath: true, customPathTemplate: "m/123'/4/{index}'/8", customPathCount: 2 }),
    ];
  ...
  it('varies only accounts, starts address indices from zero, and pads each account independently', () => {
    ...
    expect(paths.map((p) => [p.path(0), p.minimum])).toEqual([
      ["m/44'/5'/7'/0/0", 21],
      ["m/44'/5'/8'/0/0", 21],
    ]);
  ...
  for (const network of ['mainnet', 'testnet'] as const) {
    it(`runs standard Bitcoin discovery once, plus exactly two custom accounts (${network})`, async () => {
      ...
      const result = await BITCOIN_RECOVERY_ADAPTER.scan(
        { id: 'fixture', label: 'Fixture', mnemonic: MNEMONIC, passphrase: '' },
        {
          ...range,
          network,
          customPathTemplate: `m/44'/${coin}'/7'/0/{index}`,
          customPathRangeEnd: `m/44'/${coin}'/8'/0/{index}`,
    ...
    it(`runs standard Ethereum profiles once without changing their accounts (${network})`, async () => {
      const queried: string[] = [];
      await ETHEREUM_RECOVERY_ADAPTER.scan(
        { id: 'fixture', label: 'Fixture', mnemonic: MNEMONIC, passphrase: '' },
        {
          ...range,
          network,
          customPathTemplate: "m/44'/60'/7'/0/{index}",
          customPathRangeEnd: "m/44'/60'/8'/0/{index}",
    ...
    it(`scans explicit Dash paths, extends a used boundary, and resets the next account (${network})`, async () => {
      ...
      const address = (account: number, index: number) => {
        ...
        try {
          return encodeP2pkh(hash160(child.publicKey!), network === 'mainnet' ? 76 : 140);
        } finally {
          child.wipePrivateData();
        }
      };
      const used = address(7, 20);
      const queried: string[] = [];
      try {
        const gateway = new RecoveryNetworkGateway(
          new SecretEgressGuard(),
          api({
            coreStatus: async () => ({ status: 'ok' }),
            coreTip: async () => ({ resultSet: [{ height: 10 }] }),
            coreAddressInfo: async (_network, addresses) => {
              queried.push(...addresses);
              return addresses.map((address) => ({ address, balance: '0', txCount: address === used ? 1 : 0 }));
            },
          }),
        );
        const result = await scanDashCore(
          'fixture',
          seed,
          {
            ...range,
        ...
        expect(queried).toEqual([
          ...Array.from({ length: 41 }, (_, i) => address(7, i)),
          ...Array.from({ length: 21 }, (_, i) => address(8, i)),
        ]);
      } finally {
