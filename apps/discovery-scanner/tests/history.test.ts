...
const stats = (count: number, received = 1000, spent = 1000) => ({
  address: btc,
  chain_stats: { tx_count: count, funded_txo_sum: received, spent_txo_sum: spent },
  mempool_stats: { tx_count: 1 },
});
const btcTx = (id: number, time: number, incoming: number, outgoing: number) => ({
  txid: txid(id),
  status: { confirmed: true, block_time: time },
  vout: [{ scriptpubkey_address: btc, value: incoming }],
  vin: [{ prevout: { scriptpubkey_address: btc, value: outgoing } }],
});
...
describe('Bitcoin history', () => {
  it('retains used-zero lifetime totals and separates receipt, spend and pending activity', async () => {
    fetchMock((url) =>
      url.endsWith('/txs/chain') ? [btcTx(2, 1700100000, 0, 1000), btcTx(1, 1700000000, 1000, 0)] : stats(2),
    );
    const h = await bitcoinAddressHistory(btc, endpoints);
    expect(h).toMatchObject({
      status: 'complete',
      totalReceivedAtomic: '1000',
      totalSentAtomic: '1000',
  ...
  it('follows confirmed transaction cursors across pages', async () => {
    ...
    const fetch = fetchMock((url) =>
      url.endsWith(`/chain/${txid(26)}`)
        ? [btcTx(1, 1690000000, 1, 0)]
        : url.endsWith('/txs/chain')
          ? page
          : stats(26, 26, 0),
    );
    const h = await bitcoinAddressHistory(btc, endpoints);
  ...
  it('rejects duplicates, inconsistent sums and changing snapshots', async () => {
    fetchMock((url) =>
      url.includes('/txs/') ? [btcTx(1, 1700000000, 1000, 0), btcTx(1, 1700000000, 0, 1000)] : stats(2),
    );
  ...
  it('keeps full summary totals but withholds unproven dates when pagination is capped', async () => {
    ...
    fetchMock((url) =>
      url.includes('/txs/')
        ? Array.from({ length: 25 }, (_, i) => btcTx(++page, 1700000000 + i, 1, 0))
        : stats(999, 999, 0),
    );
    const h = await bitcoinAddressHistory(btc, endpoints);
...
describe('Ethereum history', () => {
  it('counts contract deployment value once when the normal recipient is null', async () => {
    ...
    fetchMock((url) => ({
      items: url.endsWith('/internal-transactions') ? [root] : [deployed],
      next_page_params: null,
    }));
    expect(await ethereumAddressHistory(eth, 'mainnet')).toMatchObject({
      status: 'complete',
      totalReceivedAtomic: '100',
      totalSentAtomic: '0',
      totalFeesAtomic: '0',
      transactionCount: 1,
  ...
  it('counts incoming/internal ETH exactly, separates gas and ignores failed transfers and duplicate root calls', async () => {
    fetchMock((url) => ({
      items: url.endsWith('/internal-transactions')
        ? [
            trace(1, 0, other, eth, '100'),
            trace(2, 1, other, eth, '7'),
            { ...trace(3, 1, other, eth, '999'), success: false, error: 'reverted' },
          ]
        : [
            ethTx(4, eth, other, '99', 'error'),
            ethTx(3, eth, eth, '2'),
            ethTx(2, eth, other, '20'),
            ethTx(1, other, eth, '100'),
          ],
      next_page_params: null,
    }));
  ...
  it('follows numeric cursors without trusting arbitrary provider URLs', async () => {
    fetchMock((url) =>
      url.endsWith('/internal-transactions')
        ? { items: [], next_page_params: null }
        : url.includes('?')
          ? { items: [ethTx(1, other, eth, '1')], next_page_params: null }
          : {
              items: [ethTx(2, eth, other, '1')],
              next_page_params: {
                block_number: 2,
                index: 1,
                items_count: 50,
                hash: `0x${txid(2)}`,
                inserted_at: '2026-09-07T19:32:38.148793Z',
                value: '0',
                fee: '5324383402476',
              },
            },
...
const report = (): RecoveryWalletResult => ({
  inputId: 'test',
  label: 'test',
  coinId: 'future',
...
const context = (): RecoveryScanContext => ({
  ...
  networkApi: {} as RecoveryScanContext['networkApi'],
  onProgress: vi.fn(),
  onFinding: vi.fn(),
});
const adapter = (getHistory?: RecoveryCoinAdapter['getHistory']): RecoveryCoinAdapter => ({
  id: 'future',
  label: 'Future coin',
  networks: ['mainnet'],
  scan: async () => report(),
...
describe('adapter history contract and exports', () => {
  it('supports a future adapter without coin switches in the runner and exports exact units', async () => {
    const h = {
      ...emptyHistory('TEST', 'tiny units', 18),
      status: 'complete' as const,
      totalReceivedAtomic: '123456789123456789123456789',
      totalSentAtomic: '0',
      firstSeen: '2024-01-01T00:00:00.000Z',
    };
    const result = report();
    await enrichRecoveryHistory(
      adapter(async () => h),
      result,
      context(),
    );
    const json = JSON.parse(createRecoveryExport([result], 'json').text);
    expect(json.results[0].sections[0].findings[0].history).toEqual(
    ...
    expect(csv).toContain('total_received_atomic');
    expect(csv).toContain(h.totalReceivedAtomic);
    expect(csv).not.toContain('first_received_utc');
    expect(csv).not.toContain('last_spent_utc');
    expect(csv).toContain('balance_atomic_unit');
    expect(json.results[0].sections[0].findings[0].balanceUnit).toEqual({
      asset: 'TEST',
      atomicUnit: 'tiny units',
      decimals: 18,
    });
    expect(csv).toContain('123456789.123456789123456789');
    expect(csv).toContain('tiny units');
  ...
  it('adds watch-only Dash Core and Platform history with correct atomic units', async () => {
    const ctx = context();
    ctx.networkApi.coreAddressHistory = vi.fn(async () => ({
      address: btc,
      txCount: 2,
      received: '123',
      sent: '123',
      firstSeenBlockTimestamp: '2024-01-01T00:00:00Z',
      lastSeenBlockTimestamp: '2024-01-02T00:00:00Z',
    }));
    const f = finding();
    expect(await getDashHistory(f, 'core', 'mainnet', ctx)).toMatchObject({
      status: 'complete',
      atomicUnit: 'duffs',
      decimals: 8,
      totalReceivedAtomic: '123',
    });
    ctx.networkApi.platformAddressHistory = vi.fn(async () => ({
      resource: btc,
      balance: '0',
      transactionCount: 2,
      incomingCount: 1,
      outgoingCount: 1,
      totalReceived: '100000000001',
      totalSent: '100000000001',
      totalFees: null,
      firstSeen: null,
      lastSeen: null,
      indexedHeight: 10,
    }));
    expect(await getDashHistory(f, 'platform', 'mainnet', ctx)).toMatchObject({
      atomicUnit: 'credits',
      decimals: 11,
      totalReceivedAtomic: '100000000001',
    });
    expect(await getDashHistory(f, 'shielded', 'mainnet', ctx)).toMatchObject({
      status: 'unsupported',
      asset: 'DASH',
      atomicUnit: 'credits',
      decimals: 11,
...
it('validates history RPC address, network and coin before network access', async () => {
  ...
  await expect(service.addressHistory('bitcoin', 'mainnet', 'https://example.test')).rejects.toThrow(
    /invalid Bitcoin/iu,
  );
  ...
  await expect(service.addressHistory('unknown' as 'bitcoin', 'mainnet', btc)).rejects.toThrow(
    'Unsupported history coin',
  );
...
it('exports explicit Dash L1/L2 units even when history cannot be loaded', async () => {
  ...
  r.sections = (
    ['core', 'legacyCore', 'coinjoin', 'providerCollateral', 'platform', 'identity', 'shielded'] as const
  ...
  for (const section of json.results[0].sections) {
    ...
    expect(section.findings[0].balanceUnit).toEqual({
      asset: 'DASH',
      atomicUnit: credit ? 'credits' : 'duffs',
      decimals: credit ? 11 : 8,
    });
...
it('rejects history expressed in the wrong unit instead of rendering a 1000x error', async () => {
  const r = report();
  r.sections[0]!.id = 'platform';
  const badAdapter = {
    ...adapter(async () => ({
      ...emptyHistory('DASH', 'duffs', 8),
      status: 'complete' as const,
      totalReceivedAtomic: '100000000000',
    })),
    amountUnit: dashAmountUnit,
  };
  await enrichRecoveryHistory(badAdapter, r, context());
  expect(r.sections[0]!.findings[0]!.history).toMatchObject({
    status: 'unavailable',
    atomicUnit: 'credits',
    totalReceivedAtomic: null,
  });
