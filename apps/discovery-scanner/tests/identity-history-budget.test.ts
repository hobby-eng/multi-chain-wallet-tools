...
function fixture(total: number) {
  ...
  vi.stubGlobal('fetch', async (url: string) => {
    ...
    if (url.endsWith('/status'))
      return Response.json({ network: 'mainnet', indexer: { status: 'synced' }, api: { block: { height: 10 } } });
...
it('rejects impossible transfer work before requesting any history pages', async () => {
  ...
  await expect(new DirectRecoveryNetworkService().platformIdentityHistory('mainnet', identifier)).rejects.toThrow(
    `${PLATFORM_IDENTITY_HISTORY_MAX_TRANSFERS}-transfer`,
  );
...
it('keeps the zero-transfer history valid', async () => {
  fixture(0);
  expect(await new DirectRecoveryNetworkService().platformIdentityHistory('mainnet', identifier)).toMatchObject({
    totalReceived: '0',
    totalSent: '0',
  });
});
