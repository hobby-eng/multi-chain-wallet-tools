...
function adapter(id: string): RecoveryCoinAdapter {
  return {
    ...
    label: id,
    networks: ['mainnet'],
    scan: vi.fn(async (input, cfg) => {
      const report = candidateFailure(input, cfg, id, id, '');
      report.warnings = [];
      report.sections[0]!.state = 'complete';
      report.sections[0]!.balanceAvailable = true;
      return report;
    }),
  };
}
...
describe('automatic ready-candidate scanning', () => {
  it('visits each candidate then each coin, preserves the phrase until all coins finish, and forces standard coverage with history', async () => {
    ...
    const coins = ['bitcoin', 'dash'].map((id) => {
      const coin = adapter(id);
      const original = coin.scan;
    ...
    expect(results.mock.calls.map(([r]) => r.inputId)).toEqual([
      'candidate-1-bitcoin',
      'candidate-1-dash',
      'candidate-2-bitcoin',
      'candidate-2-dash',
  ...
  it('does not expose invalid phrases or stop subsequent candidates', async () => {
    ...
    await scanCandidates(
      ...
      context(),
      (s) => {
        if (s === 'phrase1') throw new Error(s);
        return s;
      },
      results,
    );
  ...
  it('retains explicit failures and proceeds to the next coin without exposing provider errors', async () => {
    const broken = adapter('bitcoin');
    broken.scan = vi.fn(async () => {
      throw new Error('phrase1');
    });
    const results = vi.fn();
  ...
  it('stops between bounded operations, records cancellation and wipes all candidates', async () => {
    ...
    const coin = adapter('dash');
    coin.scan = vi.fn(async () => {
      abort.abort();
      throw new Error('stopped');
    });
    const candidates = inputs();
    const results = vi.fn();
    const later = adapter('other');
    await expect(
      scanCandidates(candidates, [coin, later], config, context(abort.signal), (s) => s, results),
    ).rejects.toMatchObject({ name: 'AbortError' });
    expect(later.scan).not.toHaveBeenCalled();
  ...
  it('does not confuse missing balances, skipped coverage, zero balances and funded partial findings', () => {
    ...
    section.findings[0]!.balanceAtomic = 1n;
