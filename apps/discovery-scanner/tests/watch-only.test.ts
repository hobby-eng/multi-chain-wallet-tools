...
import {
  ...
  parseWatchOnlyLines,
  resolveWatchOnlyTargets as resolveWatchOnlyTargetsForProfile,
...
const resolveWatchOnlyTargets = (raw: string, adapters: readonly RecoveryCoinAdapter[]) =>
  resolveWatchOnlyTargetsForProfile(
    raw,
    adapters,
    MULTI_CHAIN_WATCH_ONLY_PREFIX_COINS,
    true,
    ['ethereum'],
    (adapterId) => (adapterId === 'bitcoin' ? [3, 4] : [3, 4, 5]),
  );
...
async function scan(
  ...
  raw: string,
  ctx: RecoveryScanContext,
  scanConfig: RecoveryWatchOnlyScanConfig = config,
) {
...
describe('automatic public-key discovery', () => {
  it('treats a plain curve key as compatible with every installed coin, not as a coin identifier', () => {
    for (const raw of [publicKey, `public-key:${publicKey}`, `0x${publicKey.toUpperCase()}`]) {
      expect(resolveWatchOnlyTargets(raw, adapters).map(({ adapterId }) => adapterId)).toEqual([
        'bitcoin',
        'ethereum',
        'dash',
      ]);
    }
    expect(resolveWatchOnlyTargets(publicKey, [DASH_RECOVERY_ADAPTER]).map(({ adapterId }) => adapterId)).toEqual([
      'dash',
    ]);
  });
  it('uses explicit formats and prefixes to narrow the coin without a selector', () => {
    expect(
      resolveWatchOnlyTargets(`ethereum-xpub:${account.publicExtendedKey}`, adapters).map(({ adapterId }) => adapterId),
    ).toEqual(['ethereum']);
    expect(resolveWatchOnlyTargets(`ethereum-xpub:${account.publicExtendedKey}`, adapters)[0]?.network).toBeUndefined();
    expect(
      resolveWatchOnlyTargets(`dash-core-xpub:${account.publicExtendedKey}`, adapters).map(
        ({ adapterId }) => adapterId,
      ),
    ).toEqual(['dash']);
    expect(
      resolveWatchOnlyTargets(`dash-coinjoin-xpub:${account.deriveChild(0).publicExtendedKey}`, adapters).map(
        ({ adapterId }) => adapterId,
      ),
    ).toEqual(['dash']);
    const body = `wpkh([12345678/84h/0h/0h]${account.publicExtendedKey}/0/*)`;
    expect(
      resolveWatchOnlyTargets(`${body}#${descriptorChecksum(body)}`, adapters).map(({ adapterId }) => adapterId),
    ).toEqual(['bitcoin']);
    expect(resolveWatchOnlyTargets(`identity:${hash}`, adapters)[0]?.material.kind).toBe('identity');
    expect(() => resolveWatchOnlyTargets(`bitcoin-xpub:${account.publicExtendedKey}`, [DASH_RECOVERY_ADAPTER])).toThrow(
      /edition/u,
    );
  });
  it('preserves encoded networks and limits ambiguous xpubs to compatible derivation depths', () => {
    ...
    expect(
      resolveWatchOnlyTargets(account.deriveChild(0).publicExtendedKey, adapters).map(({ adapterId }) => adapterId),
    ).toEqual(['bitcoin', 'ethereum']);
    const testAccount = HDKey.fromMasterSeed(new Uint8Array(32).fill(8), {
      private: 0x04358394,
      public: 0x043587cf,
    }).derive("m/44'/1'/0'");
  ...
  it('rejects invalid curve points, broken checksums and master xpubs locally', () => {
    ...
    expect(() =>
      resolveWatchOnlyTargets(HDKey.fromMasterSeed(new Uint8Array(32).fill(9)).publicExtendedKey, adapters),
    ).toThrow(/master/u);
  });
  it.each([
    [0x049d7cb2, 'mainnet', 'Nested SegWit'],
    [0x04b24746, 'mainnet', 'Native SegWit'],
    [0x044a5262, 'testnet', 'Nested SegWit'],
    [0x045f1cf6, 'testnet', 'Native SegWit'],
  ] as const)(
    'preserves prefixed SLIP132 metadata and scans its encoded network (%s)',
    async (version, network, family) => {
      ...
      const query = vi.fn(async (_network, addresses: string[]) =>
        addresses.map((address) => ({ address, balance: '0', transactionCount: 0 })),
      );
      await BITCOIN_RECOVERY_ADAPTER.scanWatchOnly!(
        { ...prefixed.material, id: 'encoded', label: 'encoded' },
        { ...config, network: prefixed.network ?? 'mainnet' },
        context({ utxoAddresses: query }),
      );
  ...
  it('checks known Bitcoin addresses and exports a funded public key without blocking the report', async () => {
    ...
    const ctx = context({
      utxoAddresses: async (_network, addresses) => {
        ...
        return addresses.map((address) => ({
          address,
          balance: address === '1BgGZ9tcN4rm9KBzDn7KprQz87SZ26SAMH' ? '100' : '0',
          transactionCount: 1,
        }));
      },
    });
    ...
    for (const format of ['csv', 'json'] as const)
      expect(() =>
        ctx.sessionSecretGuard!.assertPublic(createRecoveryExport([result], format).text, 'export'),
      ).not.toThrow();
  });
  it('derives the known Ethereum address locally and sends only the address', async () => {
    const evmAccounts = vi.fn(async (_network, addresses: string[]) => ({
      blockNumber: '100',
  ...
  it.each(['h', "'"])(
    'preserves %s descriptor paths in funded findings and exports for every script family and branch',
    async (marker) => {
      ...
      for (const mode of ['legacy', 'nested-segwit', 'native-segwit', 'taproot'] as const) {
        for (const branch of [0, 1]) {
          ...
          const body = derived
            .watchOnly!.text.split('#')[0]!
            .replace(/\[[^\]]+\]/u, (origin) => origin.replaceAll('h', marker));
          ...
          expect(result.sections[0]?.findings[0]).toMatchObject({
            ...
            fields: expect.arrayContaining([
              { label: 'Relative derivation path', value: expectedPath, copyable: true },
            ]),
          });
  ...
  it('allows an explicit Identity hash lookup and converts Core duffs and Platform credits correctly', async () => {
    const identity = {
      identities: [{ identifier: '123456789ABCDEFGHJKLMNPQRSTUV', balance: '100000000000', revision: '0' }],
    ...
    const ctx = context({
      coreAddressInfo: async (_network, addresses) =>
        addresses.map((address, i) => ({ address, balance: i === 0 ? '100000000' : '0', txCount: 1 })),
      platformAddresses: async () => ({
        entries: [[`00${hash}`, { balance: '100000000000', nonce: '1' }]],
        metadata: { height: '1', protocolVersion: 1, coreChainLockedHeight: 1, timeMs: '1' },
      }),
    ...
    expect(() =>
      ctx.sessionSecretGuard!.assertPublic(createRecoveryExport([exact], 'json').text, 'export'),
    ).not.toThrow();
  });
  it('preserves Core findings when Platform services are unavailable', async () => {
    const result = await scan(
      ...
      context({
        coreAddressInfo: async (_network, addresses) =>
          addresses.map((address) => ({ address, balance: '100', txCount: 1 })),
      }),
    );
  ...
  it('recognizes a multiline Dash viewing bundle and its embedded network', () => {
    const raw = JSON.stringify(
      { format: 'dash-shielded-viewing-bundle', version: 1, network: 'testnet', fullViewingKey: '12'.repeat(96) },
      null,
      2,
    );
...
it.each([0, 1])(
  'derives Dash L2 payment addresses from DIP17 class %i xpub and keeps credits intact',
  async (classIndex) => {
    ...
    expect(() => resolveWatchOnlyTargets(publicXpub, [DASH_RECOVERY_ADAPTER])).toThrow(
      /does not encode its hardened ancestry/u,
    );
    const queries: string[] = [];
    const ctx = context({
      platformAddresses: async (_network, addresses) => {
        queries.push(...addresses);
        return {
          entries: addresses.includes(address)
            ? [[`00${bytesToHex(childHash)}`, { balance: '100000000001', nonce: '1' }]]
            : [],
          metadata: { height: '1', protocolVersion: 1, coreChainLockedHeight: 1, timeMs: '1' },
        };
      },
    ...
    expect(result.sections[0]?.findings[0]).toMatchObject({
      title: address,
      balanceAtomic: 100000000001n,
      balanceLabel: '1.00000000001 DASH',
    });
...
it('scans explicitly labelled Dash Core branch xpubs at relative child indices', async () => {
  ...
  const result = await scan(
    DASH_RECOVERY_ADAPTER,
    `dash-core-xpub:${branch.publicExtendedKey}`,
    context({
...
it('scans explicitly labelled DIP9 branch xpubs without routing them to Platform', async () => {
  ...
  const result = await scan(
    DASH_RECOVERY_ADAPTER,
    `dash-coinjoin-xpub:${branch.publicExtendedKey}`,
    context({
      coreStatus: async () => ({ status: 'ok' }),
      coreTip: async () => ({ resultSet: [{ height: 1 }] }),
      coreAddressInfo: async (_network, addresses) => {
  ...
  expect(result.sections[0]).toMatchObject({
    id: 'coinjoin',
    title: 'Dash Mobile CoinJoin · DIP9 watch-only addresses',
  });
});
...
it('checks the original checksum before deriving an apostrophe-origin descriptor', async () => {
  ...
  await expect(scan(BITCOIN_RECOVERY_ADAPTER, `${apostrophe}#${descriptorChecksum(body)}`, context())).rejects.toThrow(
    'checksum',
  );
});
...
it.each(['mainnet', 'testnet'] as const)('scans explicit legacy account and branch keys on %s', async (networkName) => {
  ...
  for (const branch of [null, 0, 1]) {
    ...
    const result = await scan(
      DASH_RECOVERY_ADAPTER,
      `dash-legacy-xpub:${publicNode.publicExtendedKey}`,
      context({
        coreStatus: async () => ({ status: 'ok' }),
        coreTip: async () => ({ resultSet: [{ height: 1 }] }),
        coreAddressInfo: async (_network, addresses) => {
  ...
  await expect(
    scan(DASH_RECOVERY_ADAPTER, `dash-legacy-xpub:${node.derive('m/0/0').publicExtendedKey}`, context(), {
      ...config,
      network: networkName,
    }),
...
it.each([BITCOIN_RECOVERY_ADAPTER, ETHEREUM_RECOVERY_ADAPTER, DASH_RECOVERY_ADAPTER])(
  'rejects an out-of-range address count before $id sends requests',
  async (adapter) => {
    await expect(scan(adapter, publicKey, context(), { ...config, minimumCount: 2147483649 })).rejects.toThrow(
      '1 to 2147483648',
    );
  },
);
...
it.each(['mainnet', 'testnet'] as const)(
  'scans the exact Dash descriptor branch on %s and rejects altered origins/checksums',
  async (network) => {
    ...
    const result = await scan(
      ...
      context({
        ...
        coreAddressInfo: async (_network, addresses) => {
          ...
          return addresses.map((address) => ({
            address,
            balance: address === expected ? '1' : '0',
            txCount: address === expected ? 1 : 0,
          }));
        },
      }),
      { ...config, network },
    ...
    expect(result.sections[0]!.findings[0]!.fields).toContainEqual({
      label: 'Descriptor derivation path',
      value: `m/44'/${net.coinType}'/7'/1/0`,
      copyable: true,
    });
    const bare = resolveWatchOnlyTargets(descriptor, adapters);
    expect(bare).toHaveLength(2);
    expect(bare.every((target) => target.ambiguity !== undefined)).toBe(true);
    expect(resolveWatchOnlyTargets(`dash-descriptor:${descriptor}`, adapters)).toHaveLength(1);
    const wrongOrigin = body.replace('/7h]', '/8h]');
    expect(() =>
      resolveWatchOnlyTargets(`dash-descriptor:${wrongOrigin}#${descriptorChecksum(wrongOrigin)}`, adapters),
    ).toThrow(/child index/u);
    ...
    expect(() =>
      resolveWatchOnlyTargets(`dash-descriptor:${privateBody}#${descriptorChecksum(privateBody)}`, adapters),
    ).toThrow();
  },
);

it.each(["m/7'", "m/9'/5'/4'/7'"])(
  'recognizes non-BIP44 Dash descriptor origin %s without deriving a different family',
  (path) => {
