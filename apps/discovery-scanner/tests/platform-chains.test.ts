...
for (const network of ['mainnet', 'testnet'] as const) {
  for (const used of [
    ...
    it(`scans ${network} hardened receive/internal classes with independent gaps: ${JSON.stringify(used)}`, async () => {
      ...
      const paths =
        network === 'mainnet' ? ["m/9'/5'/17'/0'/0'", "m/9'/5'/17'/0'/1'"] : ["m/9'/1'/17'/0'/0'", "m/9'/1'/17'/0'/1'"];
      try {
        for (const [keyClass, path] of paths.entries()) {
          ...
          try {
            for (let index = 0; index < 30; index++) {
              ...
              oracle.set(encodePlatformP2pkh(hash, network === 'mainnet' ? 'dash' : 'tdash'), {
                keyClass,
                index,
                storage: `00${bytesToHex(hash)}`,
                path: `${path}/${index}`,
              });
        ...
        const client = {
          addresses: async (addresses: string[]) => {
            ...
            return {
              entries: addresses.flatMap((address) => {
        ...
        for (const finding of section.findings) {
          ...
          expect(finding.fields).toContainEqual({
            label: 'DIP17 derivation path',
            value: expected.path,
            copyable: true,
          });
        ...
        const result = {
          ...
          sections: [section],
          warnings: [],
        } satisfies RecoveryWalletResult;
