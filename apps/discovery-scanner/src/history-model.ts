...
export function validateHistory(value: RecoveryHistory): RecoveryHistory {
  ...
  if (!value || !['complete', 'partial', 'unavailable', 'unsupported'].includes(value.status))
  ...
  if (!Number.isSafeInteger(value.decimals) || value.decimals < 0 || value.decimals > 30)
    throw new Error('Invalid history units.');
  h.decimals = value.decimals;
  for (const key of ['totalReceivedAtomic', 'totalSentAtomic', 'totalFeesAtomic'] as const) {
    const amount = value[key];
    if (amount !== null && (typeof amount !== 'string' || !PROVIDER_UNSIGNED_DECIMAL.test(amount)))
      throw new Error('Invalid history amount.');
    h[key] = amount;
  }
  for (const key of ['firstSeen', 'lastSeen', 'firstReceived', 'lastReceived', 'firstSpent', 'lastSpent'] as const) {
    const date = value[key];
    if (date !== null && (typeof date !== 'string' || !/Z$/u.test(date) || !Number.isFinite(Date.parse(date))))
...
export function historyFields(h: RecoveryHistory): RecoveryField[] {
  const fields: RecoveryField[] = [
    { label: 'History coverage', value: `${h.status}${h.scope ? ` · ${h.scope}` : ''}` },
  ];
  for (const [key, label] of [
    ['totalReceivedAtomic', 'Total received'],
    ['totalSentAtomic', 'Total sent'],
    ['totalFeesAtomic', 'Fees paid'],
  ] as const) {
  ...
  for (const [key, label] of [
    ...
    fields.push({
      label,
      value: h[key] === null ? 'Not available' : `${h[key]!.replace('T', ' ').replace(/(?:\.000)?Z$/u, '')} UTC`,
    });
  }
  if (h.transactionCount !== null) fields.push({ label: 'History transactions', value: String(h.transactionCount) });
  if (h.pendingTransactionCount !== null)
    fields.push({ label: 'Pending transactions', value: String(h.pendingTransactionCount) });
