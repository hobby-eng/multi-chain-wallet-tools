...
if (typeof window !== 'undefined') {
  window.addEventListener('message', (event: MessageEvent<unknown>) => {
    ...
    if (result.type !== RECOVERY_EXPORT_RESULT || typeof result.id !== 'string' || typeof result.ok !== 'boolean')
      return;
    ...
    if (result.ok && typeof result.filename === 'string') request.resolve(result.filename);
      ...
      request.reject(
        new Error(typeof result.error === 'string' ? result.error : 'The isolated export broker rejected the report.'),
      );
  });
}
