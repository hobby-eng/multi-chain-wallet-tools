...
export function describeCustomPath(
  template: string,
): { purpose: number; coin: number; account: number; branch: number } | null {
  try {
    ...
    return {
      purpose: Number(parts[1]!.slice(0, -1)),
      coin: Number(parts[2]!.slice(0, -1)),
      account: Number(parts[3]!.slice(0, -1)),
      branch: Number(parts[4]),
    };
  } catch {
    return null;
  }
...
export function editCustomPath(template: string, field: 'account' | 'branch', value: string): string {
  if (describeCustomPath(template) === null || !/^(0|[1-9][0-9]*)$/u.test(value) || Number(value) > MAX_BIP32_INDEX)
    return template;
  const parts = template.trim().split('/');
