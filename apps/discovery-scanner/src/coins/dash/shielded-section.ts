...
export function sectionFromLedger(
  ...
  const visibleRecords = snapshot.records.filter((record) =>
    shouldDisplayShieldedActivity(record, options.includeUsedZeroBalance),
  );
  ...
  const spendableCount = snapshot.records.filter(
    (record) => record.incoming !== undefined && record.spent === false,
  ).length;
  ...
  const findings: RecoveryFinding[] = visibleRecords.map((record) => {
    ...
    const finding: RecoveryFinding = {
      ...
      fields: [
        ...
        ...(record.spentAtPosition === undefined
          ? []
          : [{ label: 'Spent at pool position', value: record.spentAtPosition.toString() }]),
  ...
  const balanceMetric = balanceAvailable
    ? {
        ...
        value: formatDashFromCredits(snapshot.balance ?? 0n),
        tone: (snapshot.balance ?? 0n) > 0n ? ('positive' as const) : ('neutral' as const),
      }
    : {
        label: 'Spendable balance',
        value: 'Not available · not an authoritative full balance',
  ...
  return {
    ...
    metrics: [
      ...
      ...(keyKind === 'outgoing'
        ? []
        : [{ label: `${totalScope} received`, value: formatDashFromCredits(snapshot.receivedExternal ?? 0n) }]),
      ...(keyKind === 'incoming'
        ? []
        : [{ label: `${totalScope} sent`, value: formatDashFromCredits(snapshot.sentExternal ?? 0n) }]),
      ...(keyKind === 'full'
        ? [{ label: `${totalScope} self/change`, value: formatDashFromCredits(snapshot.selfOrChange ?? 0n) }]
        : []),
    ...
    ...(outcome.complete && capabilityNote === undefined
      ...
      : {
          warning: [
            ...(outcome.complete
              ? []
              : [`${incompleteReason} Results are partial; do not treat the displayed balance as authoritative.`]),
