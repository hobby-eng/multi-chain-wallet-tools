...
export function describePreviousScriptSig(
  ...
  if (scriptSig.length === 0) return { signature: null, signatureHash: null, publicKey: null, asm: 'Empty', pushes: [], raw: '' };
  let decoded;
  try { decoded = decodeScript(scriptSig, chain, network, 'spending'); }
  catch (error) {
    return {
      signature: null, signatureHash: null, publicKey: null, pushes: [], raw: scriptSig,
  ...
  const publicKey = decoded.operations.length === 2 && /^(?:02|03)[0-9a-f]{64}$|^04[0-9a-f]{128}$/u.test(decoded.operations[1]?.data ?? '')
...
export function describeOpReturn(script: Uint8Array, chain: PsbtChain, network: PsbtNetwork): OpReturnDetails | null {
  let decoded;
  try { decoded = decodeScript(Array.from(script, (value) => value.toString(16).padStart(2, '0')).join(''), chain, network, 'script-pubkey'); }
  catch { return null; }
