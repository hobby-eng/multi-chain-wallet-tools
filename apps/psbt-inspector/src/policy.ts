...
function validatePublicKeys(values: readonly string[]): Uint8Array[] {
  ...
  return values.map((value, index) => {
    ...
    if (/^[xt]pub[1-9a-hj-np-za-km-z]+$/u.test(normalized) || /^\[[0-9a-f]{8}\/[^]+\][xt]pub/u.test(normalized)) {
      throw new Error(
        `Public key ${index + 1} is an account xpub, not a compressed child public key. Use Multisig wallet -> Deterministic ranged wallet for xpubs, or paste a derived 33-byte child public key that starts with 02 or 03.`,
      );
    }
    if (!/^(02|03)[0-9a-f]{64}$/u.test(normalized))
      throw new Error(`Public key ${index + 1} must be a compressed 33-byte secp256k1 key.`);
    ...
    try {
      ...
      throw new Error(`Public key ${index + 1} is not a valid secp256k1 point.`);
    }
    return bytes;
  });
}
...
function orderKeyBytes(keys: readonly Uint8Array[], keyOrder: 'supplied' | 'bip67'): Uint8Array[] {
  return [...keys].sort((left, right) =>
    keyOrder === 'bip67' ? bytesToHex(left).localeCompare(bytesToHex(right)) : 0,
  );
}
...
function lockDetails(kind: LockKind, value: number): { fragment: string; requirement: string } {
  ...
  if (!Number.isSafeInteger(value) || value < 0 || value > 0x7fffffff)
    throw new Error('Lock value is outside the supported script-number range.');
  if (kind === 'height') {
    if (value >= CONSENSUS_LIMITS.absoluteLockTimeThreshold)
      throw new Error('Absolute block height must be below 500000000; larger values are timestamps.');
    fragmentName = 'after';
    requirement = `Absolute block height ≥ ${value}; spending input sequence must not be final`;
  } else if (kind === 'time') {
    if (value < CONSENSUS_LIMITS.absoluteLockTimeThreshold)
      throw new Error('Absolute Unix locktime must be at least 500000000.');
    fragmentName = 'after';
...
function signingMiniscript(keys: readonly Uint8Array[], required: number, tapscript: boolean): string {
  return keys.length === 1 && required === 1
    ? `pk(${bytesToHex(keys[0]!)})`
    : multisigMiniscript(keys, required, tapscript);
}
...
export function buildPolicy(request: PolicyRequest): BuiltPolicy {
  ...
  if (musig2) {
    ...
    if (mode !== 'locked-multisig' || request.lockKind !== 'none') {
      throw new Error(
        'MuSig2 key-path construction is a cooperative N-of-N spend and cannot represent the selected script policy or timelock.',
      );
    }
    if (request.required !== sourceKeys.length) {
      throw new Error(
        `MuSig2 key-path construction requires all ${sourceKeys.length} participants; it is not M-of-N multisig.`,
      );
    }
    ...
    return {
      ...
      compatibility:
        'Bitcoin Taproot key-path MuSig2 output using a BIP390 musig() descriptor. Spending requires a signer that implements the MuSig2 nonce and partial-signature protocol; ordinary PSBT multisig signing is not sufficient.',
      keyOrder,
  ...
  if (mode === 'htlc') {
    if (request.lockKind === 'none')
      throw new Error('An HTLC-like fallback branch requires an absolute or relative timelock.');
    ...
    if (request.lockKind === 'none')
      throw new Error('A delayed recovery branch requires an absolute or relative timelock.');
    ...
    if (request.lockKind === 'none')
      throw new Error('A delayed recovery or backup branch requires an absolute or relative timelock.');
    const recoveryKeys = orderedPolicyKeys(request.recoveryPublicKeys ?? [], keyOrder, tapscript);
    const recoveryRequired = request.recoveryRequired ?? Math.min(request.required, recoveryKeys.length);
    const delayed = lockDetails(request.lockKind, request.lockValue);
    requirement =
    ...
    if (recoveryRequired >= request.required)
      throw new Error('Decaying multisig must reduce the required signature count after the lock.');
    const delayed = lockDetails(request.lockKind, request.lockValue);
    ...
    if (duplicate !== undefined)
      throw new Error(`Expanding multisig additional key duplicates a primary key: ${bytesToHex(duplicate)}.`);
    ...
    if (keys.length !== 1 || request.required !== 1)
      throw new Error('Staged recovery requires exactly one primary key and one required primary signature.');
    ...
    if (recoveryKey === undefined || (request.recoveryPublicKeys?.length ?? 0) !== 1)
      throw new Error('Staged recovery requires exactly one first recovery key.');
    if (emergencyKey === undefined || (request.emergencyPublicKeys?.length ?? 0) !== 1)
      throw new Error('Staged recovery requires exactly one second recovery key.');
    ...
    policyExpression =
      request.lockKind === 'none' ? primaryExpression : `after(${request.lockValue},${primaryExpression})`;
  ...
  if (
    ...
    redeemScript.length > CONSENSUS_LIMITS.maximumScriptElementBytes
  ) {
    throw new Error(
      'This P2SH redeem script exceeds the 520-byte script element limit. Use fewer public keys or Bitcoin P2WSH.',
    );
  }
  ...
  if (request.chain === 'bitcoin' && request.bitcoinWrapper === 'p2tr') {
    ...
    address = bech32.encode(request.network === 'mainnet' ? 'bc' : request.network === 'regtest' ? 'bcrt' : 'tb', [
      0,
      ...bech32.toWords(digest),
    ]);
  } else {
  ...
  const keyOrderDescription = tapscript
    ? keyOrder === 'bip67'
      ...
      : 'x-only keys committed exactly as entered to multi_a()'
    : keyOrder === 'bip67'
      ? 'BIP67 lexicographic sortedmulti'
      : 'committed exactly as entered';
  return {
    redeemScript,
    scriptPubKey,
    address,
