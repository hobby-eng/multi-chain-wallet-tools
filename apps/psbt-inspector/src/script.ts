...
function scriptNumber(hex: string): number | null {
  ...
  let value = 0;
  for (let index = 0; index < bytes.length; index += 1)
    value += (bytes[index]! & (index === bytes.length - 1 ? 0x7f : 0xff)) * 2 ** (8 * index);
...
function parse(hexText: string): { bytes: Uint8Array; operations: ScriptOperation[] } {
  ...
  if (!/^[0-9a-f]+$/u.test(hex))
    throw new Error('Malformed Script hex: only hexadecimal characters and whitespace are allowed.');
  ...
  while (offset < bytes.length) {
    ...
    if (opcode === 0x4c) {
      if (offset >= bytes.length) throw new Error('Malformed Script: truncated OP_PUSHDATA1.');
      length = bytes[offset++]!;
    }
    if (opcode === 0x4d) {
      if (offset + 2 > bytes.length) throw new Error('Malformed Script: truncated OP_PUSHDATA2.');
    ...
    if (length !== null) {
      if (length > bytes.length - offset)
        throw new Error(`Malformed Script: push at byte ${start} exceeds the script length.`);
      data = bytesToHex(bytes.slice(offset, offset + length));
      offset += length;
    }
    operations.push({
      offset: start,
      opcode,
      name:
        data === null ? (NAMES[opcode] ?? `OP_UNKNOWN_0x${opcode.toString(16).padStart(2, '0')}`) : `PUSH(${length})`,
      data,
...
function multisigAt(ops: readonly ScriptOperation[], start: number): { end: number; text: string } | null {
  ...
  let index = start + 1;
  let keys = 0;
  while (ops[index]?.data !== null && /^(02|03)[0-9a-f]{64}$/u.test(ops[index]?.data ?? '')) {
    keys += 1;
    index += 1;
  }
...
function lockAt(ops: readonly ScriptOperation[], start: number): { next: number; condition: string } | null {
  ...
  const condition =
    lockOp === 0xb1
      ? `absolute lock ${lock}`
      : (lock & CONSENSUS_LIMITS.bip68TypeFlag) !== 0
        ? `relative delay ${(lock & CONSENSUS_LIMITS.bip68SequenceMask) * 512} seconds`
        : `relative delay ${lock & CONSENSUS_LIMITS.bip68SequenceMask} blocks`;
...
function infer(ops: readonly ScriptOperation[]): string {
    ...
    if (primary !== null && ops[primary.end]?.opcode === 0x67) {
      ...
      if (ops[primary.end + 1]?.opcode === 0x63) {
        ...
        if (first !== null && ops[first.end]?.opcode === 0x67) {
          ...
          if (
            second !== null &&
            ops[second.end]?.opcode === 0x68 &&
            ops[second.end + 1]?.opcode === 0x68 &&
            second.end + 2 === ops.length
          ) {
  ...
  if (
    lock !== null &&
    (ops[1]?.opcode === 0xb1 || ops[1]?.opcode === 0xb2) &&
    (ops[2]?.opcode === 0x69 || ops[2]?.opcode === 0x75) &&
    locked?.end === ops.length
  )
    return `${locked.text} after lock value ${lock}`;
...
export function decodeScript(
  ...
  if (role === 'spending') {
    ...
    if (bytes.length <= CONSENSUS_LIMITS.maximumScriptElementBytes && p2shDescription.address !== null)
      wrappers.push({
        label: `${chain === 'dash' ? 'Dash' : 'Bitcoin'} P2SH`,
        address: p2shDescription.address,
        scriptPubKey: bytesToHex(p2sh),
      });
    if (chain === 'bitcoin' && bytes.length <= CONSENSUS_LIMITS.maximumScriptBytes) {
      ...
      if (description.address !== null)
        wrappers.push({ label: 'Bitcoin P2WSH', address: description.address, scriptPubKey: bytesToHex(p2wsh) });
    }
  }
  return {
    hex: bytesToHex(bytes),
    byteLength: bytes.length,
    asm: operations.map((op) => (op.data === null ? op.name : pushedValueAsm(op.data))).join(' '),
    operations,
    classification: direct.type,
    directAddress: direct.address,
    inferredPolicy: infer(operations),
    wrappers,
  };
}
