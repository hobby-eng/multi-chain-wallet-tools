...
function inputValue(map: readonly PsbtPair[], txInput: TransactionInput | undefined, chain: PsbtChain): bigint | null {
  ...
  if (previous === undefined) return witness?.value ?? null;
...
function validateVersionFields(global: readonly PsbtPair[], inputs: readonly (readonly PsbtPair[])[], outputs: readonly (readonly PsbtPair[])[], version: number): void {
  if (version === 0) {
  ...
  for (const item of map) {
    if ((scope === 'input' && item.type === 0x1an) || (scope === 'output' && item.type === 0x08n)) {
      ...
      for (let offset = 0; offset < item.value.length; offset += 33) {
        validateCompressedPublicKey(item.value.slice(offset, offset + 33), `PSBT ${scope} MuSig2 participant ${offset / 33 + 1}`);
      }
...
export function parsePsbt(text: string, chain: PsbtChain): ParsedPsbt {
  ...
  validateVersionFields(global, inputs, outputs, version);
  const inputValues = inputs.map((map, index) => inputValue(map, transaction?.inputs[index], chain));
  ...
  return { chain, version, global, inputs, outputs, transaction, inputValues, outputValues, fee };
}
