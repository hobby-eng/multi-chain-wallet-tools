...
const script = (d: string, branch: 0 | 1 = 0, index = 0) =>
  decodeDescriptor(d, { multipathChoice: branch, wildcardIndex: index }).compiledOutput?.rows.find(
    (r) => r.label === 'scriptPubKey',
  )?.value;
...
describe('follow-up: original MuSig grammar before branch selection', () => {
  ...
  it('preserves valid nested origins and participant-only multipath', () => {
    expect(script(`tr(${g},{pk(musig([deadbeef/44h/0h]${g},${h})),pk(${h})})`)).toBe(
      script(`tr(${g},{pk(musig(${g},${h})),pk(${h})})`),
    );
    for (const branch of [0, 1] as const)
      expect(script(`tr(musig(${a}/<0;1>,${b}/<0;1>))`, branch)).toBe(
        script(`tr(musig(${a}/${branch},${b}/${branch}))`),
      );
  });
...
describe('follow-up: descriptor recognition and legacy contexts', () => {
  it('preserves uncompressed legacy serialization (independent Python/OpenSSL fixture)', () => {
    ...
    expect(script(`sh(pkh(${u}))`)).toBe(
      'a914' +
        hash(
          'ripemd160',
          hash('sha256', Buffer.from('76a91491b24bf9f5288532960ac687abb035127b1d28a588ac', 'hex')),
        ).toString('hex') +
        '87',
    );
    expect(decodeDescriptor(`sh(pkh(${g}))`).compiledOutput?.rows.find((r) => r.label === 'Address')?.value).toBe(
...
const kv = (key: Buffer, value: Buffer) =>
  Buffer.concat([Buffer.from([key.length]), key, Buffer.from([value.length]), value]);
...
function psbt(type: number, key: Buffer, value: Buffer): string {
  return Buffer.concat([
    ...
    map([f(251, u32(2)), f(2, u32(2)), f(4, Buffer.from([1])), f(5, Buffer.from([1]))]),
    map([f(14, Buffer.alloc(32)), f(15, u32(0)), kv(Buffer.concat([Buffer.from([type]), key]), value)]),
    map([f(3, Buffer.alloc(8)), f(4, Buffer.from([81]))]),
  ]).toString('base64');
}
...
describe('follow-up: BIP174 preimage commitments (OpenSSL oracle)', () => {
  for (const [type, name, digest] of [
    ...
    for (const value of [Buffer.alloc(0), Buffer.from('public preimage')])
      it(`accepts ${name} length ${value.length}`, () => {
    ...
    it(`rejects malformed ${name} key widths and wrong commitments`, () => {
      const value = Buffer.from('public preimage'),
        key = digest(value);
      for (const wrong of [key.subarray(1), Buffer.concat([key, Buffer.from([0])]), Buffer.alloc(key.length)])
        expect(() => parsePsbt(psbt(type, wrong, value), 'bitcoin')).toThrow(/preimage/);
    });
  }
});
