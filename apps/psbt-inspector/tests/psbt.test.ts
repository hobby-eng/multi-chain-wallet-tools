...
describe('Bitcoin regtest address rendering', () => {
  it('uses bcrt for witness addresses while retaining testnet Base58 versions', () => {
    expect(describeScript(hexToBytes('0014d85c2b71d0060b09c9886aeb815e50991dda124d'), 'bitcoin', 'regtest')).toEqual({
      type: 'P2WPKH',
...
describe('PSBT inspector core', () => {
  it('compiles the fixed Bitcoin Core P2WSH descriptor into concrete Bitcoin data', () => {
    const descriptor = decodeDescriptor(
      'wsh(or\\_d(pk(03a34b99f22c790c4e36b2b3c2c35a36db06226e41c692fc82b8b56ac1c540c5bd),and\\_v(v:pk(02e8a3647fe4637f85c3f80c101a995803c78e674f8a644281b3ae0bcc8e215833),older(12960))))#80auchz3',
    );
    expect(descriptor.ranged).toBe(false);
    expect(descriptor.compiledOutput).not.toBeNull();
    expect(descriptor.compiledOutput?.asm).toBe(
      '<03a34b99f22c790c4e36b2b3c2c35a36db06226e41c692fc82b8b56ac1c540c5bd> OP_CHECKSIG OP_IFDUP OP_NOTIF <02e8a3647fe4637f85c3f80c101a995803c78e674f8a644281b3ae0bcc8e215833> OP_CHECKSIGVERIFY 12960 OP_CHECKSEQUENCEVERIFY OP_ENDIF',
    );
  ...
  it('preserves multi() key order in the supplied Bitcoin Core HTLC descriptor', () => {
    ...
    expect(descriptor.compiledOutput?.rows).toContainEqual({
      ...
      value:
        '',
    });
    expect(descriptor.compiledOutput?.rows).toContainEqual({
      label: 'scriptPubKey',
      value: '00209ef2009ca41ef41fb85141aca8b6d3275bd4966fdc20f8624c7568ec152b5337',
    });
    expect(descriptor.compiledOutput?.rows).toContainEqual({
      label: 'Address',
      value: 'bc1qnmeqp89yrm6plwz3gxk23dknyadaf9n0mss0scjvw45wc9ft2vms4v4hea',
    });
  });

  it.each([
    [
      'BIP-381 compressed pk',
      'pk(03a34b99f22c790c4e36b2b3c2c35a36db06226e41c692fc82b8b56ac1c540c5bd)',
      '2103a34b99f22c790c4e36b2b3c2c35a36db06226e41c692fc82b8b56ac1c540c5bdac',
    ],
    [
      'BIP-381 uncompressed pk',
  ...
  it('matches the official ranged BIP-382 wpkh outputs', () => {
    ...
    expected.forEach((scriptPubKey, wildcardIndex) => {
      expect(decodeDescriptor(source, { wildcardIndex }).compiledOutput?.rows).toContainEqual({
        label: 'scriptPubKey',
        value: scriptPubKey,
      });
    });
  ...
  it('explains constants and wrapper identities in a Miniscript descriptor', () => {
    ...
    expect(descriptor.rows).toContainEqual({
      label: 'Wrapper n:',
      value: expect.stringContaining('canonical true or false'),
    });
  });
  ...
  it('builds custom Bitcoin P2WSH and Tapscript policies through pinned libraries', () => {
    ...
    expect(() => buildDashDisabledCustomPolicy('', 'p2wsh', 'mainnet')).toThrow(
      /unavailable in the Dash Community build/u,
    );
  });
  ...
  it('summarizes a ranged Taproot Miniscript descriptor and its CSV time lock', () => {
    const xpub =
      'xpub6ERApfZwUNrhLCkDtcHTcxd75RbzS1ed54G1LkBUHQVHQKqhMkhgbmJbZRkrgZw4koxb5JaHWkY4ALHY2grBGRjaDMzQLcgJvLJuZZvRcEL';
    const descriptor = decodeDescriptor(
      `tr(${xpub}/0/*,{multi_a(2,[deadbeef/87h/0h/0h]${xpub}/0/*,[feedface/87h/0h/0h]${xpub}/0/*),and_v(v:multi_a(2,[deadbeef]${xpub}/1/*,[feedface]${xpub}/1/*,[01020304]${xpub}/0/*),older(4194339))})`,
    );
  ...
  it('summarizes a Liana-style ranged wsh recovery descriptor with multipath xpubs', () => {
    ...
    expect(descriptor.rows).toContainEqual({
      ...
      value:
        'sane · non-malleable satisfactions available · signature required · compatible timelock units · no duplicate keys',
    });
    expect(descriptor.compiledOutput?.rows).toContainEqual({ label: 'Output type', value: 'P2WSH' });
    const change = decodeDescriptor(source, { multipathChoice: 1, wildcardIndex: 17 });
    expect(change.compiledOutput?.rows.find(({ label }) => label === 'Address')?.value).not.toBe(
      descriptor.compiledOutput?.rows.find(({ label }) => label === 'Address')?.value,
    );
  ...
  it('rejects invalid BIP-390 placement and mixed derivation modes', () => {
    ...
    expect(() => decodeDescriptor(`rawtr(musig(${BIP390_XPUBS[0]}/0/*,${BIP390_XPUBS[1]})/0/*)`)).toThrow(
      /does not allow participant wildcard/u,
    );
    expect(() => decodeDescriptor(`rawtr(musig(${BIP390_XPUBS.join(',')})/0'/0)`)).toThrow(/cannot contain hardened/u);
    expect(() => decodeDescriptor(`rawtr(musig(${BIP390_XPUBS.join(',')})/*/*)`)).toThrow(
      /at most one final wildcard/u,
    );
    expect(() => decodeDescriptor(`rawtr(musig(${BIP390_XPUBS.join(',')}):1)`)).toThrow(
      /Unexpected MuSig2 key suffix/u,
    );
  });
  ...
  it('supports BIP-390 singleton aggregation and participant-level wildcard derivation', () => {
    ...
    const participantDerived = analyzeMusigDescriptor(
      `rawtr(musig(${BIP390_XPUBS[0]}/0/*,${BIP390_XPUBS[1]}/0/*))`,
      'mainnet',
      2,
    );
  ...
  it('counts a musig() expression as one multi_a signing key', () => {
    ...
    const descriptor = decodeDescriptor(
      `tr(${internal},multi_a(1,musig(${DASH_CORE_BIP67_VECTORS[0].keys.join(',')})))`,
    );
  ...
  it('inspects a BIP-390 musig() key inside a Silent Payments descriptor without inventing an address', () => {
    ...
    expect(decodeDescriptor(`sp(${BIP390_XPUBS[0]},musig(${BIP390_XPUBS.join(',')}))`).classification).toBe(
  ...
  it('names and explains BIP-373 MuSig2 PSBT fields', () => {
    ...
    expect(pairSummary('input', { type: 0x1an, keyData: aggregate, value: participants })).toContain(
      '2 compressed participant',
    );
    expect(() =>
      validateMusigPsbtFields([{ type: 0x1an, keyData: aggregate.slice(1), value: participants }], 'input'),
    ).toThrow(/33-byte compressed/u);
    expect(() =>
      validateMusigPsbtFields([{ type: 0x1bn, keyData: participants, value: new Uint8Array(65) }], 'input'),
    ).toThrow(/exactly 66 bytes/u);
    expect(() =>
      validateMusigPsbtFields([{ type: 0x1cn, keyData: participants, value: new Uint8Array(31) }], 'input'),
  ...
  it('decodes HTLC-like hashlock and timelocked fallback paths', () => {
    const descriptor = decodeDescriptor(
      `wsh(andor(pk(${DASH_CORE_BIP67_VECTORS[0].keys[0]}),sha256(9595c9df90075148eb06860365df33584b75bff782a510c6cd4883a419833d50),and_v(v:pk(${DASH_CORE_BIP67_VECTORS[0].keys[1]}),older(144))))`,
    );
  ...
  it('builds an HTLC-like policy through the Miniscript compiler', () => {
    ...
    expect(policy.miniscript).toBe(
      `andor(pk(${DASH_CORE_BIP67_VECTORS[0].keys[0]}),sha256(9595c9df90075148eb06860365df33584b75bff782a510c6cd4883a419833d50),and_v(v:older(144),pk(${DASH_CORE_BIP67_VECTORS[0].keys[1]})))`,
    );
  ...
  it('builds the exact staged key-recovery shape through the Miniscript compiler', () => {
    ...
    expect(policy.miniscript).toBe(
      `or_d(pk(${keys[0]}),or_i(and_v(v:pkh(${keys[1]}),older(12960)),and_v(v:pkh(${keys[2]}),older(52560))))`,
    );
  ...
  it('normalizes escaped underscores and wildcards in staged ranged descriptors', () => {
    const descriptor = decodeDescriptor(
      `wsh(or\\_d(pk([deadbeef/48'/0'/0'/2']${BIP390_XPUBS[0]}/<0;1>/\\*),or\\_i(and\\_v(v:pkh([cafebabe/48'/0'/1'/2']${BIP390_XPUBS[1]}/<0;1>/\\*),older(12960)),and\\_v(v:pkh([01020304/48'/0'/2'/2']${BIP390_XPUBS[0]}/<0;1>/\\*),older(52560)))))`,
    );
    expect(descriptor.ranged).toBe(true);
  ...
  it('keeps Dash descriptor inspection on legacy Script and rejects Bitcoin-only wrappers', () => {
    ...
    const legacy = decodeDescriptor(
      `sh(and_v(v:pk(${key}),sha256(9595c9df90075148eb06860365df33584b75bff782a510c6cd4883a419833d50)))`,
      { chain: 'dash' },
    );
    ...
    expect(legacy.rows).toContainEqual({
      label: 'Hashlock 1',
      value:
        'SHA-256 digest 9595c9df90075148eb06860365df33584b75bff782a510c6cd4883a419833d50 · spending requires the exact 32-byte preimage',
    });
    expect(() => decodeDescriptor(`wsh(pk(${key}))`, { chain: 'dash' })).toThrow(
  ...
  it('decodes the BIP174 creator vector and its outputs', () => {
    ...
    expect(transactionId(parsed.transaction!.raw)).toBe(
    ...
    expect(decodeScript(bytesToHex(output!.script), 'bitcoin', 'mainnet', 'script-pubkey').asm).toBe(
      'OP_0 d85c2b71d0060b09c9886aeb815e50991dda124d',
    );
  });
  ...
  it.each(['bitcoin', 'dash'] as const)(
    'retains a decoded non-witness UTXO for complete %s transaction display',
    (chain) => {
  ...
  it('makes concrete supplied-order and BIP67 descriptors explicit', () => {
    ...
    expect(decoded.compiledOutput?.rows.some(({ label, value }) => label === 'Address' && value.startsWith('7'))).toBe(
      true,
    );
  });
  ...
  it('still rejects xpubs in the low-level concrete public-key-only policy builder', () => {
    expect(() =>
      buildPolicy({
        ...
        required: 1,
        publicKeys: [
          'xpub6EHhWk3orDvW8dgKQVfpFCV1WSY2iqmNxkNYQRA76Sp4X3rD3kfSf6Bp6PsCHcTV4vZWGZDDRXs6UhX9HCwFxK8poPZP1GLAdHqVns5NtWj',
        ],
        lockKind: 'none',
        lockValue: 0,
        bitcoinWrapper: 'p2sh',
  ...
  it('builds a ranged Bitcoin P2SH watch-only wallet from account xpubs', () => {
    ...
    const wallet = buildRangedWallet({
      ...
      accountXpubs: accounts.map(
        (account, index) => `[${index === 0 ? 'aaaaaaaa' : 'bbbbbbbb'}/48h/0h/0h/2h]${account.publicExtendedKey}`,
      ),
      branches: [0, 1],
      startIndex: 0,
      endIndex: 1,
    ...
    expect(wallet.rows[0]?.publicKeys).toEqual(
      accounts.map((account) => bytesToHex(account.deriveChild(0).deriveChild(0).publicKey!)),
    );
  ...
  it('explains mainnet xpub versus testnet tpub mismatches', () => {
    const account = HDKey.fromMasterSeed(new Uint8Array(32).fill(61), getDashNetwork('mainnet').versions).derive(
      "m/48'/5'/0'/0'",
    );
    expect(() =>
      buildRangedWallet({
  ...
  it('builds and decodes a 4-of-5 or delayed recovery policy', () => {
    ...
    expect(() =>
      buildPolicy({
        ...
        recoveryPublicKeys: [keys[0]!, keys[1]!],
      }),
  ...
  it('builds and decodes a delayed R-of-K recovery multisig policy', () => {
    ...
    expect(decodeScript(policyHex(policy).redeemScript, 'bitcoin', 'testnet', 'spending').inferredPolicy).toContain(
      '2-of-3 multisig after relative delay 144 blocks',
    );
  });
  ...
  it('builds and decodes a backup committee policy', () => {
    ...
    expect(decodeScript(policyHex(policy).redeemScript, 'dash', 'testnet', 'spending').inferredPolicy).toContain(
      '1-of-2 multisig after absolute lock 2000000',
    );
  });
  ...
  it('builds every preset as a single-leaf Taproot Tapscript policy', () => {
    ...
    const policies = [
      ...
      buildPolicy({
        ...
        recoveryRequired: 1,
        hashKind: 'sha256',
        hashDigest: '11'.repeat(32),
      }),
      buildPolicy({
        ...common,
        mode: 'staged-recovery',
        required: 1,
        publicKeys: keys.slice(0, 1),
        lockKind: 'relative-blocks',
        lockValue: 144,
        recoveryPublicKeys: keys.slice(1, 2),
        secondLockKind: 'relative-blocks',
        secondLockValue: 1008,
        emergencyPublicKeys: keys.slice(2, 3),
      }),
      buildPolicy({
        ...common,
        mode: 'delayed-recovery',
        ...
        lockKind: 'relative-blocks',
        lockValue: 144,
        recoveryPublicKey: keys[3]!,
        recoveryPublicKeys: keys.slice(3, 4),
      }),
      buildPolicy({
        ...common,
        mode: 'delayed-recovery-multisig',
        ...
        lockKind: 'relative-blocks',
        lockValue: 144,
        recoveryPublicKeys: keys.slice(3, 6),
        recoveryRequired: 2,
      }),
      buildPolicy({
        ...common,
      ...
      buildPolicy({
        ...
        secondLockKind: 'relative-blocks',
        secondLockValue: 1008,
        emergencyPublicKeys: keys.slice(5, 7),
        emergencyRequired: 1,
      }),
    ];
  ...
  it('warns that Dash Core cannot automatically sign the custom recovery template', () => {
    const secrets = [1, 2, 3, 4].map((value) => {
      const secret = new Uint8Array(32);
      secret[31] = value;
      return bytesToHex(secp256k1.getPublicKey(secret, true));
    });
    const policy = buildPolicy({
      ...
      network: 'mainnet',
      required: 2,
      publicKeys: secrets.slice(0, 3),
      lockKind: 'relative-blocks',
      lockValue: 20,
      bitcoinWrapper: 'p2sh',
      mode: 'delayed-recovery',
      recoveryPublicKey: secrets[3]!,
    });
    expect(policy.compatibility).toContain('ADVANCED CUSTOM DASH P2SH');
  ...
  it('rejects a non-empty scriptSig in a PSBT v0 unsigned transaction', () => {
    const invalid = BIP174_CREATOR.replace('01009a', '01009b').replace(
      'dd750000000000ffffffff',
      'dd75000000000151ffffffff',
    );
...
describe('Audit regressions: cryptographic context and output limits', () => {
  ...
  it('emits a 32-byte CHECKSIG key for a valid Tapscript', () => {
    expect(
      bytesToHex(buildCustomMiniscriptPolicy(`multi_a(1,${generator.slice(2)})`, 'tapscript', 'mainnet').script),
  ...
  it('rejects a timestamp entered as a block height', () => {
    expect(() =>
      buildPolicy({
        ...
        network: 'mainnet',
        required: 1,
        publicKeys: [generator],
        lockKind: 'height',
        lockValue: 500_000_000,
        bitcoinWrapper: 'p2wsh',
      }),
    ).toThrow(/height/iu);
  });
