# Wallet Activity Viewer security boundary

- This application is intentionally online. Do not enter a mnemonic, BIP39 passphrase, spending key or private key.
- Raw 32-byte Orchard data is accepted as OVK only after an explicit advanced selection because its length cannot distinguish it from a spending key.
- The viewing key remains local, but a remote provider can observe the source IP, query timing and complete-pool scan pattern.
- Bitcoin, Ethereum, Dash Core/Platform address, and Identity queries reveal those public identifiers and their timing to the selected providers.
- Bitcoin and Ethereum balances/history are third-party indexed or RPC data rather than locally verified consensus state. Bounded or incomplete history is labelled partial and unavailable totals or dates are not inferred.
- Core indexed history is not consensus-proofed. Platform address/Identity current state and encrypted-note pages use DAPI proofs; indexed Platform history is separately trusted and cross-checked where possible.
- All CSV/XLSX/JSON exports contain privacy-sensitive activity metadata even though they contain no viewing key. XLSX generation is local and separates addresses, Identities, Orchard records, and errors into worksheets. Single and Batch exports are available for every included adapter; normalized public-address exports also exclude private input material.
- An Orchard balance/spent state is authoritative only after scanning from pool position zero through two proof-verified empty terminal reads under the bounded stream policy. Decreasing proof heights are rejected, including on empty pages; the documented append-only commitment-tree assumption still applies. A short non-empty response is processed and followed by another request; it is never treated as the pool end.
- All query types start disabled and fail closed until the embedded Orchard runtime passes fixed public derivation, canonical-key, malformed-key, and scanner-boundary checks and the Blob Worker check succeeds.
- The connected CSP permits `worker-src blob:` because the pinned official Evo SDK creates a local compilation worker from its already embedded JavaScript. Remote and sibling worker URLs remain blocked.
- Evo SDK 4.1.1 is distributed as a combined read/write facade rather than a separate read-only browser entry point. This Viewer accepts no spending authority, calls only proof/read methods, and its release verifier rejects source calls to known transfer, withdrawal, identity-write, or broadcast methods. It also rejects imports of the SDK wallet API and calls to its reviewed mnemonic/seed methods, so a future refactor cannot silently introduce Evo-backed secret handling into the connected graph. Unused write and wallet symbols still contribute to artifact size and dependency attack surface.
- The Orchard WASM build remaps private machine paths; release verification rejects `/home/<user>`, `/Users/<user>`, and Windows user-profile paths in the binary.

See the root [security audit](../../SECURITY_AUDIT.md) for provider and host-compromise limitations.

- Coin, network, inputs and mode controls share the query busy state. Clear/Cancel target the operation that owns the request, and late replies cannot restore cleared results. HTTP timeouts and abort relay remain active through response-body parsing.
- Core and Platform history keep fixed page sizes. Repeated address transaction IDs, changing totals and premature short pages fail the lookup. Identity collection paging likewise keeps stable offsets and rejects repeated pages/changing totals. These checks detect inconsistencies; they do not turn an index into a consensus proof.
