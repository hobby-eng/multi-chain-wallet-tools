...
describe('key derivation BIP-322 signer', () => {
  it('signs native SegWit simple messages', async () => {
    ...
    expect(
      (await verifyBip322Message('native SegWit fixture', payment.address, signed.signature, 'mainnet')).valid,
    ).toBe(true);
  });
  ...
  it('signs nested SegWit full messages', async () => {
    ...
    expect(
      (await verifyBip322Message('nested SegWit fixture', payment.address, signed.signature, 'mainnet')).valid,
    ).toBe(true);
  });
  ...
  it('signs Taproot key-path simple messages', async () => {
    ...
    expect(signed.format).toBe('BIP-322 simple');
    expect((await verifyBip322Message('Taproot fixture', payment.address, signed.signature, 'mainnet')).valid).toBe(
      true,
    );
  });
});
