...
Offline, standalone BIP39/BIP32/ZIP-32 derivation. The local Seed Diagnostic reports word count, English-wordlist membership, checksum validity, NFKD normalization, entropy/checksum sizes, and the passphrase-sensitive BIP32 master fingerprint; unknown English words receive bounded nearest-word suggestions. Its reveal-gated construction view lists every word position, BIP39 index in decimal/hex, the corresponding 11-bit group, canonical entropy in hex/binary, and checksum bits; validation and entropy recovery use the pinned `@scure/bip39` implementation. The input validator and secure phrase generator support every standard English BIP39 length: 12, 15, 18, 21, and 24 words. The extensible Multi-Chain edition currently includes Bitcoin, Ethereum, and Dash (Core BIP44, legacy mobile Core, Platform payments, Platform Identity keys, Purpose48 P2SH multisig cosigner keys, and Orchard). Dash Community is Dash-only and compiles only those six Dash adapters. Build outputs: `dist/multi-chain-edition/key-derivation/Wallet_Key_Derivation_Tool.html` and `dist/dash-community-edition/key-derivation/Dash_Community_Key_Derivation_Tool.html`.
...
The shared **Recovery & Backup** workspace is present in both editions. The original phrase or current BIP85 child phrase can be passed to a selected recovery workflow through an expiring in-memory reference, without clipboard or storage. The Derive view can also reveal the concealed BIP39 entropy; it never treats the separate passphrase as entropy. Wallet Matcher numbers seed phrases, their aligned passphrases, and known addresses as separate records; pasted line breaks inside a valid phrase are normalized only when its BIP39 checksum proves the boundary. It accepts aligned lists of BIP39 phrases and passphrases plus one or more known addresses, narrows compatible profiles from each address unless forced to scan all, and reports the matching seed number and full derivation path within explicit account/index bounds. SeedQR supports Standard numeric and Compact binary formats for every standard 12/15/18/21/24-word English BIP39 phrase. SeedSigner documents 12/24-word payloads; intermediate lengths round-trip inside this tool but may not be accepted by every external SeedQR implementation. SLIP-39 uses the current Trezor-compatible mnemonic format. CKD Shamir Raw and CKD Shamir Words use the same pinned Rust `sharks` GF(256) shares. New cards omit visible format prefixes; a checksummed internal envelope still carries version and set metadata, and legacy prefixed cards remain decodable. The word form uses BIP39 vocabulary but is not a BIP39 mnemonic or SLIP-39 share. Codex32 follows draft BIP93 and its native GF(32) sharing scheme. Each generated SeedQR, SLIP-39 share, Shamir share, or Codex32 record is rendered as its own offline QR code with local PNG export. Every restore mode accepts local QR image files; CompactSeedQR import preserves raw bytes including NUL and line-break values.

SLIP-39 and both CKD Shamir modes back up the entropy of a valid English BIP39 phrase and restore those words. Codex32 can store either BIP39 entropy, which reconstructs the original English words, or the 64-byte BIP32 master seed produced from the phrase and its exact Unicode passphrase. Master-seed mode restores those seed bytes, but the PBKDF2 conversion is one-way and cannot recover the original words or passphrase. All share creation uses `crypto.getRandomValues`; Shamir expands a 256-bit browser seed with ChaCha20Rng, and Codex32 receives independent browser-generated random field-point payloads. No recovery method uses `Math.random`, network access, or browser storage.
...
The main UI loads protocol metadata only. All runtime key derivation and the Orchard WASM instance execute in a disposable Blob worker. The offline Recovery & Backup workspace uses separate embedded Shamir and Codex32 WASM modules on the main thread because it does not share derivation-worker state. Message signing, BIP38 encryption, BIP85 child-secret derivation, and BIP352 scan/spend-key derivation use the same disposable-worker boundary. The final HTML has `connect-src 'none'`, no external assets, no storage APIs and no runtime install requirement.
...
Both editions include coin-independent BIP85 child BIP39 derivation for all standard 12/15/18/21/24-word lengths. A child mnemonic opens as an in-memory wallet workspace with the edition’s supported coins, the normal result table, private-key reveal, clipboard controls, and account descriptor export; no URL, storage, popup, or network transport is used. Both editions additionally expose XPRV and hexadecimal BIP85 applications; WIF choices follow the edition (Dash only in Dash Community, Bitcoin and Dash in Multi-Chain). Multi-Chain also includes BIP352 Silent Payment address derivation. Silent Payments show the canonical scan and spend paths, base address, and reserved `m=0` change label. This offline utility does not scan chain history: detecting payments still requires eligible transaction input data and ordered outputs.
...
## Feature-selective standalone builds

Canonical builds include every feature allowed by their edition profile. Custom builds are written under `dist/custom-builds/<profile>/` and never replace release artifacts.

```bash
pnpm build:key-derivation -- --profile multi-chain --features derive,bip85,seedqr
pnpm build:key-derivation -- --profile dash-community --exclude slip39,shamir,codex32
pnpm build:key-derivation -- --profile multi-chain --features derive,bip85,seedqr,slip39 --exclude slip39
```

`--features` starts from the named allowlist; `--exclude` starts from the edition's complete set and removes names. When both are present, exclusions are applied after the allowlist. The base `derive` feature is required. Available names are `derive`, `bip85`, `silent-payments`, `wallet-matcher`, `seedqr`, `slip39`, `shamir`, and `codex32`. `silent-payments` is available only in the multi-chain edition. Use `--output path/file.html` to choose a custom output path.
