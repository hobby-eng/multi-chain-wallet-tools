...
export function createKeyDerivationController(view: KeyDerivationView, dependencies: KeyDerivationDependencies) {
  ...
  return {
    start(): void {
        ...
        const labels = trimmed
          ...
          .map((part) => {
            ...
            if (!Number.isSafeInteger(label) || label < 1 || label > 0xffffffff) {
              throw new Error(
                `"${part}" is not a valid label. Use whole numbers from 1 to 4294967295, separated by commas.`,
              );
            }
            return label;
          });
      ...
      optionalElement<HTMLSelectElement>('#silent-payment-network')?.addEventListener(
        'change',
        scheduleSilentPaymentRefresh,
      );
      optionalElement<HTMLInputElement>('#silent-payment-account')?.addEventListener(
        'input',
        scheduleSilentPaymentRefresh,
      );
      optionalElement<HTMLInputElement>('#silent-payment-labels')?.addEventListener(
        'input',
        scheduleSilentPaymentRefresh,
      );
      ...
      toggleBip85ChildPassphraseButton?.addEventListener('click', () =>
        setBip85ChildPassphraseVisibility(!bip85ChildPassphraseRevealed),
      );
      ...
      deriveBip85Button?.addEventListener('click', () => {
        ...
        void (async () => {
          ...
          try {
            ...
            if (
              wifEncoding !== undefined &&
              wifEncoding !== 'dash-mainnet' &&
              wifEncoding !== 'dash-testnet' &&
            ...
            const derived = await worker.deriveBip85(seed, {
              ...
              ...(wifEncoding === undefined
                ? {}
                : {
                    wifEncoding: wifEncoding as 'bitcoin-mainnet' | 'bitcoin-testnet' | 'dash-mainnet' | 'dash-testnet',
                  }),
            });
      ...
      for (const selector of ['#bip85-index', '#bip85-words', '#bip85-bytes', '#bip85-wif-encoding']) {
        optionalElement<HTMLInputElement | HTMLSelectElement>(selector)?.addEventListener(
          'input',
          scheduleBip85Refresh,
        );
      }
      ...
      function clearResults(): void {
        ...
        bulkBip38Revision += 1;
        activeBulkBip38Worker?.terminate(
          new DerivationCancelledError('BIP38 encryption cancelled because results changed.'),
        );
      ...
      function messageSigningFormat(resultId: string): MessageSigningFormat | null {
        ...
        if (resultId === 'dash-core' || resultId === 'dash-legacy-mobile' || resultId === 'dash-core-coinjoin')
          return 'dash-compact';
        return null;
      }
      ...
      function openMessageSignerForResult(
        ...
        const chooseLegacyFormat =
          !__DASH_COMMUNITY__ && result.id === 'bitcoin-legacy' && messageSignerFormatField !== null;
        if (messageSignerFormatField !== null) messageSignerFormatField.hidden = !chooseLegacyFormat;
        if (!__DASH_COMMUNITY__ && messageSignerFormatSelect !== null)
          messageSignerFormatSelect.value = 'bitcoin-bip322-legacy';
      ...
      if (typeof signMessageButton?.addEventListener === 'function')
        signMessageButton.addEventListener('click', () => {
          ...
          const signingFormat =
            !__DASH_COMMUNITY__ && context?.resultId === 'bitcoin-legacy' && messageSignerFormatSelect !== null
          ...
          void (async () => {
            ...
            try {
              ...
              const { includeChange, includeCoinJoin, ...baseInput } = input;
              const plan = planResultBranches(signingAdapter, baseInput.branch, includeChange, includeCoinJoin).find(
                (candidate) => candidate.kind === context.resultBranch,
              );
      ...
      messageSignerFormatSelect?.addEventListener('change', () => {
        if (messageSignerFormatOutput !== null) {
          messageSignerFormatOutput.textContent = messageSigningLabel(
            messageSignerFormatSelect.value as MessageSigningFormat,
          );
        }
      });

      if (typeof copyMessageSignature?.addEventListener === 'function')
        copyMessageSignature.addEventListener('click', () => {
      ...
      function startBulkBip38Encryption(): void {
        ...
        void (async () => {
          ...
          const worker = createWorker();
          activeBulkBip38Worker?.terminate(
            new DerivationCancelledError('Superseded by a new BIP38 encryption request.'),
          );
