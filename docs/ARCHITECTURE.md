...
| --- | --- | --- | --- | --- |
| Wallet Key Derivation Tool | `apps/key-derivation` | `dist/multi-chain-edition/key-derivation/Wallet_Key_Derivation_Tool.html` | CSP forbids all connections | Mnemonic, passphrase, private/spending/viewing keys |
...
| PSBT Inspector | `apps/psbt-inspector` | `dist/multi-chain-edition/psbt-inspector/PSBT_Inspector.html` | CSP forbids all connections | Public PSBT/script/descriptor/policy/xpub material only; private keys and signing are out of scope |

Each application owns its HTML template, UI controller, app-specific styles, build script, artifact verifier, tests, README, and security notes. Most reusable code lives in shared packages. The current Multi-Chain Activity Viewer also imports the Discovery Scanner network/history modules; these cross-application imports are an existing implementation dependency, not an enforced isolation boundary. Dash Community uses its separate Dash entry point. PSBT Inspector is currently a Multi-Chain-only offline artifact because it intentionally exposes Bitcoin and Dash Core transaction/script/watch-only multisig workflows together.
