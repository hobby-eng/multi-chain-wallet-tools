...
> Historical pre-remediation findings. See [the remediation record](AUDIT_REMEDIATION_2026-09-12.md) for subsequent changes, test results and corrections to two oracle descriptions. External evidence paths below identify the original review environment.
...
- **Actual source:** `/home/sergio/.copilot/repos/copilot-worktrees/derivationtool/hobby-eng-studious-tribble`.
...
The older Documents checkout is now `/home/sergio/Documents/derivationtool23`; its old pathname no longer exists. The latest worktree's Git metadata points into that renamed checkout. Some other worktrees still have stale `.git` paths. They were not repaired or used as the audit target.
