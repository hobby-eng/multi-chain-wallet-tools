# AUD-006 — Documentation, integration and test-coverage follow-up — 2026-09-12
...
- **Working tree:** Substantial uncommitted changes; HEAD alone is not the reviewed snapshot.
...
| ------------------- | -------- | ------- | -------- | --------------- | ------------------------------------------------------------ |
| AUD-006-FUN001      | FUN      | finding | medium   | verified        | MuSig multipath is validated inconsistently                  |
| AUD-006-FUN002      | FUN      | finding | medium   | verified        | a MuSig key inside a Taproot tree bypasses origin validation |
| AUD-006-FUN003      | FUN      | finding | medium   | verified        | known PSBT hash-preimage fields are not validated            |
| AUD-006-FUN004      | FUN      | finding | low      | verified        | valid legacy uncompressed P2SH-P2PKH compilation fails       |
| AUD-006-FUN005      | FUN      | finding | low      | verified        | recognized-only descriptor forms accept malformed arguments  |
| AUD-006-BLD001      | BLD      | finding | low      | verified        | Docker wrapper exits can skip cleanup                        |

## Review evidence

**Latest status:** confirmed findings AUD-006-FUN001, AUD-006-FUN002, AUD-006-FUN003, AUD-006-FUN004, AUD-006-FUN005, AUD-006-BLD001 are fixed; see the remediation addendum at the end. Original discovery evidence is retained below.
...
Exact public reproduction inputs, observed outputs, extra-check results and link statuses are in [the evidence record](audit-06-2026-09-12.json). They contain only synthetic/public test material. Existing user mnemonics/xpubs were not used.

## Findings at discovery (remediation status below)

<a id="c01--medium-musig-multipath-is-validated-inconsistently"></a>

### AUD-006-FUN001 — Medium — MuSig multipath is validated inconsistently
...
<a id="c02--medium-a-musig-key-inside-a-taproot-tree-bypasses-origin-validation"></a>

### AUD-006-FUN002 — Medium — a MuSig key inside a Taproot tree bypasses origin validation
...
<a id="c03--medium-known-psbt-hash-preimage-fields-are-not-validated"></a>

### AUD-006-FUN003 — Medium — known PSBT hash-preimage fields are not validated
...
<a id="c04--low-valid-legacy-uncompressed-p2sh-p2pkh-compilation-fails"></a>

### AUD-006-FUN004 — Low — valid legacy uncompressed P2SH-P2PKH compilation fails
...
<a id="c05--low-recognized-only-descriptor-forms-accept-malformed-arguments"></a>

### AUD-006-FUN005 — Low — recognized-only descriptor forms accept malformed arguments
...
<a id="c06--low-docker-wrapper-exits-can-skip-cleanup"></a>

### AUD-006-BLD001 — Low — Docker wrapper exits can skip cleanup
...
| Descriptors/Miniscript/Taproot | Core literal script vectors, selected BIP341 trees, BIP327/328 and BIP390 outputs                                  | AUD-006-FUN001, AUD-006-FUN002, AUD-006-FUN003, AUD-006-FUN004, AUD-006-FUN005; cross-product of tree positions, wrappers, origins, sorting, network, branch and index  |
| PSBT                           | 133 BIP174/370/371/373 corpus cases, adversarial UTXO binding and shape tests                                      | AUD-006-FUN003, full recognized-field schema matrix and malformed control-block/script commitments with honest unsupported states                                       |
...
1. **Consolidate descriptor grammar first.** `descriptor.ts` (801 lines) and `musig-descriptor.ts` use separate delimiters, expression handling and key traversal. This duplication is connected to AUD-006-FUN001/AUD-006-FUN002, not merely a style preference. Keep a typed parsed tree with origin/suffix/range information; validate it once before branch selection, then compile and render from it.
...
| -------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ |
| AUD-006-FUN001 | Preserve aggregate suffix parsing; validate original participant paths before selection; reuse the public BIP32 suffix validator for synthetic aggregate xpubs. Both branches, indices 0/7/2147483647, mixed participant/aggregate ranges and malformed suffixes are tested. Go KeyAgg + Go HD derivation + concrete Go Taproot output provide independent comparisons. | Fixed  |
| AUD-006-FUN002 | Every MuSig participant passes shared origin/key/network validation, including nested leaves and the direct analysis entry point. Valid origins preserve outputs; malformed/oversized origins and opposite-network keys reject.                                                                                                                                         | Fixed  |
| AUD-006-FUN003 | All four BIP174 input preimage fields validate hash width and actual commitment; field names are explicit. Node/OpenSSL provides independent hashes for empty and nonempty valid values, wrong widths and wrong commitments.                                                                                                                                            | Fixed  |
| AUD-006-FUN004 | ASM serialization accepts 65-byte HASH160 arguments only after contextual public-key validation. Legacy uncompressed/compressed results remain different; SegWit/Tapscript rejection tests remain. The Go normalization limitation is corrected above.                                                                                                                  | Fixed  |
| AUD-006-FUN005 | Silent Payments recognition requires two key expressions; combo requires one valid key. Missing, invalid and nested non-key arguments reject; valid recognized forms remain accepted without claiming compilation/import support.                                                                                                                                       | Fixed  |
| AUD-006-BLD001 | Subprocess failures throw through cleanup; the outer handler sets the exit code. Fake-Docker tests assert container removal, empty temporary directory and unchanged previous output on copy failure or missing edition; build failure is covered too.                                                                                                                  | Fixed  |
...
Structured checks, source fingerprint, artifact hashes and individual browser outcomes are retained in [the evidence record](audit-06-2026-09-12.json), under `remediation`. Full local browser reports/screenshots are in `test-results/browser-files/2026-09-12T04-24-07.364Z/` and `test-results/browser-regressions/2026-09-12T04-26-24.921Z/`. No Docker build, native Rust run, WASM regeneration, funded transaction or publication was performed in this correction pass.
