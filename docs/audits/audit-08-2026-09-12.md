# AUD-008 — Independent-audit remediation verification — 2026-09-12
...
- **Reviewed commit:** d978dd4de68e6b51743d8ce0f01fcd6128d88553.
- **Working tree:** clean.
...
| ------------------- | -------- | --------- | -------- | --------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| AUD-007-SEC001      | SEC      | follow-up | high     | verified        | multi()/sortedmulti() with >20 keys compiles to an unspendable P2WSH output labelled sane                                                            |
| AUD-007-FUN001      | FUN      | follow-up | medium   | verified        | Firefox page crash when an in-flight derivation worker is terminated (Clear/Cancel/tab switch)                                                       |
| AUD-007-UI001       | UI       | follow-up | low      | verified        | sortedmulti shows a contradictory supplied-order row; sortedmulti_a described as order preserved                                                     |
| AUD-007-FUN002      | FUN      | follow-up | low      | verified        | PSBT cross-field relationships accepted without verification                                                                                         |
| AUD-007-BLD001      | BLD      | follow-up | low      | verified        | Dash Community artifact content gate skips the PSBT Inspector; bundle carries unreachable Bitcoin code and copy                                      |
| AUD-007-BLD002      | BLD      | follow-up | medium   | verified        | Independent conformance suite skips 56 valid Core Miniscript contexts, never compares TESTMODE flags, and accepts any error for invalid PSBT vectors |
...
- **Purpose:** verify the corrections made after `audit-07-2026-09-12`, including AUD-007-SEC001 through AUD-007-BLD002, the audit's code-quality recommendations, browser lifecycle handling, build provenance and documentation status.
