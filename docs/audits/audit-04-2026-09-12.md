# AUD-004 — Independent audit of the local wallet tools — 2026-09-12
...
- **Working tree:** Dirty working tree: 46 modified tracked files and 55 untracked files.
...
| ------------------- | -------- | ------- | -------- | --------------- | ---------------------------------------------------------------------------------------------------------- |
| AUD-004-SEC001      | SEC      | finding | high     | open            | custom Tapscript accepts the wrong public-key type and reports a signature requirement it does not enforce |
| AUD-004-FUN001      | FUN      | finding | medium   | open            | Inspector displays a verification result for an obsolete message, including after Clear                    |
| AUD-004-FUN002      | FUN      | finding | medium   | open            | PSBT parsing is substantially more permissive than the advertised versions' field schemas                  |
| AUD-004-FUN003      | FUN      | finding | medium   | open            | displayed PSBT fee is calculated from unbound or conflicting UTXO claims                                   |
| AUD-004-FUN004      | FUN      | finding | medium   | open            | descriptor validation permits invalid contexts and malformed tree/multipath syntax                         |
| AUD-004-FUN005      | FUN      | finding | medium   | open            | mixed-case Dash Platform addresses are normalized before checksum validation                               |
| AUD-004-FUN006      | FUN      | finding | medium   | open            | the Dash discovery overview double-counts an address found in overlapping L1 families                      |
| AUD-004-API001      | API      | finding | medium   | open            | Dash Core history pagination can skip confirmed records and accepts incomplete page validation             |
| AUD-004-API002      | API      | finding | medium   | open            | some Dash explorer responses are not strictly bound to the requested resource/network                      |
| AUD-004-FUN007      | FUN      | finding | medium   | open            | an absolute “block height” can compile as a timestamp lock                                                 |
| AUD-004-UI001       | UI       | finding | medium   | open            | two Deriver UI regressions hide or interrupt successfully requested operations                             |
| AUD-004-BLD001      | BLD      | finding | medium   | open            | reproducible Docker wrapper checks an obsolete output path                                                 |
| AUD-004-BLD002      | BLD      | finding | medium   | open            | the uncommitted audit suite and canonical check are not runnable as delivered                              |
| AUD-004-FUN008      | FUN      | finding | low      | open            | several valid descriptor/Miniscript paths remain inspection-only or unsupported                            |
| AUD-004-DOC001      | DOC      | finding | low      | open            | a BIP45 documentation shortcut omits a meaningful derivation level                                         |

## Review evidence

> Historical pre-remediation findings. See [the remediation record](audit-05-2026-09-12.md) for subsequent changes, test results and corrections to two oracle descriptions. External evidence paths below identify the original review environment.
...
| ---------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Mixed-case DIP18 rejection                           | Absent; mixed-case input is accepted, AUD-004-FUN005.                                                                                                                      |
| Cross-family L1 balance deduplication                | Absent; duplicated value is summed twice, AUD-004-FUN006.                                                                                                                  |
| Full Dash history page validation                    | Absent; malformed suffixes/metadata accepted, AUD-004-API001.                                                                                                              |
| Resource/network response binding                    | Address binding and strict network identity remain deficient, AUD-004-API002.                                                                                              |
| Pending-row pagination handling                      | Confirmed record can be skipped, AUD-004-API001.                                                                                                                           |
| Edition-specific Docker copy-out paths               | Old flat `release/SHA256SUMS` assumption remains, AUD-004-BLD001.                                                                                                          |
...
| Deriver signer revision fix from the supplied report | Present; do not re-report that resolved defect. Inspector verification has a distinct outstanding race, AUD-004-FUN001.                                                    |
...
<a id="f01--high-custom-tapscript-accepts-the-wrong-public-key-type-and-reports-a-signature-requirement-it-does-not-enforce"></a>

### AUD-004-SEC001 — High — custom Tapscript accepts the wrong public-key type and reports a signature requirement it does not enforce
...
<a id="f02--medium-inspector-displays-a-verification-result-for-an-obsolete-message-including-after-clear"></a>

### AUD-004-FUN001 — Medium — Inspector displays a verification result for an obsolete message, including after Clear
...
<a id="f03--medium-psbt-parsing-is-substantially-more-permissive-than-the-advertised-versions-field-schemas"></a>

### AUD-004-FUN002 — Medium — PSBT parsing is substantially more permissive than the advertised versions' field schemas
...
<a id="f04--medium-displayed-psbt-fee-is-calculated-from-unbound-or-conflicting-utxo-claims"></a>

### AUD-004-FUN003 — Medium — displayed PSBT fee is calculated from unbound or conflicting UTXO claims
...
<a id="f05--medium-descriptor-validation-permits-invalid-contexts-and-malformed-treemultipath-syntax"></a>

### AUD-004-FUN004 — Medium — descriptor validation permits invalid contexts and malformed tree/multipath syntax
...
<a id="f06--medium-mixed-case-dash-platform-addresses-are-normalized-before-checksum-validation"></a>

### AUD-004-FUN005 — Medium — mixed-case Dash Platform addresses are normalized before checksum validation
...
<a id="f07--medium-the-dash-discovery-overview-double-counts-an-address-found-in-overlapping-l1-families"></a>

### AUD-004-FUN006 — Medium — the Dash discovery overview double-counts an address found in overlapping L1 families
...
<a id="f08--medium-dash-core-history-pagination-can-skip-confirmed-records-and-accepts-incomplete-page-validation"></a>

### AUD-004-API001 — Medium — Dash Core history pagination can skip confirmed records and accepts incomplete page validation
...
<a id="f09--medium-some-dash-explorer-responses-are-not-strictly-bound-to-the-requested-resourcenetwork"></a>

### AUD-004-API002 — Medium — some Dash explorer responses are not strictly bound to the requested resource/network
...
<a id="f10--medium-an-absolute-block-height-can-compile-as-a-timestamp-lock"></a>

### AUD-004-FUN007 — Medium — an absolute “block height” can compile as a timestamp lock
...
<a id="f11--medium-two-deriver-ui-regressions-hide-or-interrupt-successfully-requested-operations"></a>

### AUD-004-UI001 — Medium — two Deriver UI regressions hide or interrupt successfully requested operations
...
<a id="f12--medium-buildreliability-reproducible-docker-wrapper-checks-an-obsolete-output-path"></a>

### AUD-004-BLD001 — Medium — reproducible Docker wrapper checks an obsolete output path
...
<a id="f13--medium-verificationrelease-the-uncommitted-audit-suite-and-canonical-check-are-not-runnable-as-delivered"></a>

### AUD-004-BLD002 — Medium — the uncommitted audit suite and canonical check are not runnable as delivered
...
<a id="f14--low--capability-gap-several-valid-descriptorminiscript-paths-remain-inspection-only-or-unsupported"></a>

### AUD-004-FUN008 — Low — several valid descriptor/Miniscript paths remain inspection-only or unsupported
...
<a id="f15--low-a-bip45-documentation-shortcut-omits-a-meaningful-derivation-level"></a>

### AUD-004-DOC001 — Low — a BIP45 documentation shortcut omits a meaningful derivation level
...
| BIP67; `multi`/`sortedmulti`      | Supplied versus sorted order is explicitly represented; official/Core literals and existing Dash BIP67 tests pass for covered valid paths. Builder duplicate checks are present. Parser/size inconsistencies remain AUD-004-FUN004.                                                                                                                                                                                                |
| BIP68/112                         | Relative units/range handling reviewed and tested by existing suites; height/time boundary defect AUD-004-FUN007. This audit did not mine timelocked transactions.                                                                                                                                                                                                                                                                 |
| BIP174/370/371                    | Fresh valid/invalid PSBT corpus plus independent malformed maps/lengths/fee probes. Significant schema gaps remain AUD-004-FUN002/AUD-004-FUN003.                                                                                                                                                                                                                                                                                  |
...
| BIP340                            | 19 official valid/invalid Schnorr verification vectors pass through Noble. This does not prove all UI-message binding, which fails AUD-004-FUN001.                                                                                                                                                                                                                                                                                 |
| BIP341                            | Seven wallet vectors pass for output script/address, Merkle root where present, and exact control-block bytes. Go independently agrees on tweaked output keys. Application constructor validation still fails AUD-004-SEC001.                                                                                                                                                                                                      |
| BIP342; `multi_a`/`sortedmulti_a` | Valid compilation literals exercised, but context validation fails AUD-004-SEC001/AUD-004-FUN004; some hash-key serialization fails AUD-004-FUN008. Full script execution was not independently performed for every emitted path.                                                                                                                                                                                                  |
...
| BIP380–386                        | Checksums, origins, standard wrappers, keys, script trees, and wildcard paths reviewed against official texts and selected cross-implementation outputs. Malformed grammar/context acceptance remains AUD-004-FUN004. Generic `tr()` compilation is incomplete.                                                                                                                                                                    |
...
| BIP389                            | Two-branch multipath derivation exists; repeated tuple acceptance remains AUD-004-FUN004. Arbitrary multi-branch Cartesian expansion is not certified.                                                                                                                                                                                                                                                                             |
...
The Inspector README appropriately warns that experimental outputs should not hold real funds. That warning is useful, but it does not resolve contradictory per-output safety claims such as AUD-004-SEC001. The BIP45 shorthand needs clarification, and unsupported compilation paths should be documented as a support matrix rather than implied by a list of BIP names.
...
- Four CoinJoin visibility failures: confirmed application defect AUD-004-UI001.
- Two Chromium BIP38 encryption failures: independently reproduced AUD-004-UI001.
...
1. Block unsafe custom Tapscript construction (AUD-004-SEC001), then add byte-level and execution-level regressions before re-enabling it.
2. Fix Inspector verification revision binding (AUD-004-FUN001).
3. Introduce strict PSBT schemas and UTXO binding; clearly separate decoded claims from verified values (AUD-004-FUN002, AUD-004-FUN003).
4. Unify descriptor/context/tree validation and correct timelock classification (AUD-004-FUN004/AUD-004-FUN007).
5. Restore the missing Dash validation, pagination, and accounting safeguards after checking current code rather than blindly cherry-picking old changes (AUD-004-FUN005, AUD-004-FUN006, AUD-004-API001, AUD-004-API002).
6. Fix the two Deriver UI regressions (AUD-004-UI001).
7. Investigate the repeatable Firefox BIP85 child-wallet crash and finish the child-wallet Clear/reload acceptance flow. Repair verification harnesses and build copy-out paths (AUD-004-BLD001, AUD-004-BLD002), then resolve/document unsupported paths and documentation gaps (AUD-004-FUN008, AUD-004-DOC001).
