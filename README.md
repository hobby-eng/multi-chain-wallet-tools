...
- **Recover & Back Up** helps you prepare and restore wallet backups locally, without sending the recovery phrase to a server. Choose whether to split the secret between several cards, encrypt it in a container, or encode it for easier transfer. Encoding alone does not hide the secret.
- **Wallet Matcher** helps identify the right phrase, BIP39 passphrase, and derivation path when you know a wallet address. It checks the candidates you supply within the selected account, branch, and index ranges.
- **SLIP-39** splits the recovery phrase into word cards: for example, create three cards and require any two to restore it. Store the cards separately so one lost or stolen card is not enough to reconstruct the phrase. This tool restores the original BIP39 words; importing the cards directly into a native SLIP-39 wallet can produce a different wallet.
- **CKD Shamir** also splits the phrase into cards, shown either as words or compact text. It is this project's custom format: restore these cards with a compatible CKD implementation, not an ordinary wallet or a SLIP-39 decoder. The format is documented in [`docs/CKD_SHAMIR_FORMAT.md`](docs/CKD_SHAMIR_FORMAT.md).
- **SSKR** splits the phrase using Blockchain Commons' standard format. You can create several groups of cards, each with its own minimum number of cards, and choose how many complete groups recovery requires. Cards can be displayed as Compact UR text for QR transfer or full Bytewords for transcription.
- **Gordian Seed Envelope** puts the phrase into an encrypted backup container. You can configure alternative ways to open the same container: a password, an authorized recipient's private key, or enough SSKR cards. It can also include the separate BIP39 passphrase; doing so puts both wallet secrets in one backup. A recipient key can be derived from a mnemonic and its BIP39 passphrase, but a key derived from the backed-up phrase cannot rescue you if that phrase is lost.
- **Codex32** makes a text backup with an error-detecting checksum, optionally split into several shares. Choose phrase backup to recover the original BIP39 words, or master-seed backup to preserve the wallet seed produced by the phrase and its BIP39 passphrase. Master-seed backup cannot recover the original words or passphrase. Codex32 encoding by itself is not encryption.
- **SeedQR** turns the recovery phrase into a QR code for offline transfer or printing. Standard mode represents word indexes as decimal digits; Compact mode stores the entropy bytes. Neither mode encrypts the phrase: anyone who reads the QR code can recover it.
- **MHFE** is this project's experimental, memory-hard encryption construction. It turns any standard English BIP39 phrase into one checksum-valid 24-word encrypted container, supports an optional public PIM work factor, QR export/import, cancellation, and explicit recovery-verifier reporting. For 24-word sources, an optional long-running profile preserves the complete final word and reports progress while it repeatedly applies the full permutation. See the [specification](https://github.com/hobby-eng/mhfe-spec) and [reference implementation](https://github.com/hobby-eng/mhfe). It has not received enough independent cryptographic review for protecting real funds.
- Each generated backup card or record can also be shown as a QR code and saved as a PNG image. This avoids retyping long records when restoring them. The restore tabs can read QR image files offline; a QR code has the same secrecy requirements as the data it contains.
- You can use the original phrase or a BIP85 child phrase directly in the backup tabs without copying it to the clipboard. The link expires when the source phrase or passphrase changes. SLIP-39, CKD Shamir, and standalone SSKR back up the phrase's entropy, so keep its separate BIP39 passphrase safe as well.
...
The project has extensive automated checks but has not received an independent cryptography-specialist audit. See the [security policy and private reporting instructions](SECURITY.md) and the [security model and current limitations](SECURITY_AUDIT.md).
