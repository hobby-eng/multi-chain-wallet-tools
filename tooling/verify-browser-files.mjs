...
const output =
  ...
  resolve(root, 'test-results/browser-files', new Date().toISOString().replaceAll(':', '-'));
...
const fixedP2wshDescriptor =
  'wsh(or_d(pk(03a34b99f22c790c4e36b2b3c2c35a36db06226e41c692fc82b8b56ac1c540c5bd),and_v(v:pk(02e8a3647fe4637f85c3f80c101a995803c78e674f8a644281b3ae0bcc8e215833),older(12960))))#80auchz3';
...
const report = {
  ...
  scope:
    'Direct file URLs, Chromium/Firefox, startup, edition selection, basic UI/derivation/clear, mocked address lookup, opaque vault isolation, scrolling and screenshots. No live-provider or real-funds test.',
  runs: [],
};
const save = () =>
...
function providerFixture(request) {
  ...
  if (url.hostname === 'blockchain.info' && url.pathname === '/balance') {
    return Object.fromEntries(
      url.searchParams
        .get('active')
        .split('|')
        .map((address) => [address, { final_balance: 0, n_tx: 0 }]),
    );
  }
  ...
  if (url.hostname === 'ethereum-rpc.publicnode.com') {
    ...
    const reply = (item) => ({
      jsonrpc: '2.0',
      id: item.id,
      result: item.method === 'eth_blockNumber' ? '0x10' : '0x0',
    });
  ...
  if (url.hostname.includes('dashscan')) {
    if (url.pathname === '/addresses/info')
      return url.searchParams
        .get('addresses')
        .split(',')
        .map((address) => ({ address, balance: '0', txCount: 0 }));
    ...
    if (url.pathname.includes('/address/'))
      return { address: url.pathname.split('/address/')[1], balance: '0', received: '0', sent: '0', txCount: 150 };
  }
        ...
        await context.route(/^https?:\/\//u, async (route) => {
          ...
          if (pauseNextDashStatus && new URL(request.url()).pathname === '/status') {
            ...
            await new Promise((resolve) => {
              releaseDashStatus = resolve;
            });
            await route.abort('aborted').catch(() => {});
            return;
          }
          try {
            await route.fulfill({ json: providerFixture(request), headers: { 'access-control-allow-origin': '*' } });
          } catch (error) {
            run.unexpectedRequests.push(String(error));
            await route.abort();
          }
        });
        try {
          ...
          if (toolId === 'discovery-scanner') {
            ...
            assert.equal(
              await scope.locator('body').evaluate(() => {
                try {
            ...
            assert.equal(
              await scope.locator('#custom-path-range-end').inputValue(),
              initialPath.replace("/0'/0/{index}", "/1'/0/{index}"),
            );
            ...
            await scope
              .locator('#watch-only-keys')
              .fill(
                "dash-descriptor:pkh([c7fe8acb/44'/1'/0']tpubDDgGSmowbmYWepHK5PJYCfzUFrKy1c7PHVumScWELYwwjaGBf73ZD1JD1xc2y4hKQDp4qHUKjxz8HQyJXmM5UQh797enQQSpq8vife8yva8/0/*)#tz4w30l2",
              );
            ...
            run.checks.push(
              'Scanner descriptor scan, pending-request cancellation, restart and secret-free JSON download',
            );
            ...
            assert.match(
              ...
              /Fee \/ current unsigned size[\s\S]*2\.57 (?:sat|duffs)\/vB[\s\S]*Transaction ID[\s\S]*Serialized size[\s\S]*Virtual size[\s\S]*Weight[\s\S]*scriptPubKey ASM/u,
            );
            assert.match(
              await page.locator('#psbt-summary').innerText(),
              /Signing state[\s\S]*Unsigned[\s\S]*UTXO information/u,
            );
            ...
            assert.match(
              await page.locator('#transaction-details').innerText(),
              /Referenced previous output[\s\S]*Matches current input prevout/u,
            );
            const previousTransaction = page.locator('.previous-transaction-details');
            await previousTransaction.locator(':scope > summary').click();
            assert.match(
              await previousTransaction.innerText(),
              /Complete previous transaction[\s\S]*84a4992f[\s\S]*Previous transaction input 0[\s\S]*Previous transaction output 0[\s\S]*P2PKH[\s\S]*Previous transaction output 1[\s\S]*P2SH[\s\S]*Raw transaction/u,
            );
            const advanced = page.locator('.psbt-advanced-details');
            assert.equal(await advanced.getAttribute('open'), null);
            await advanced.locator(':scope > summary').click();
            assert.match(await page.locator('#map-details').innerText(), /Non-witness UTXO[\s\S]*01000000/u);
            await page.locator('[data-mode="verify"]').click();
            await page
              .locator('#verify-address')
              .fill(profile.id === 'dash-community' ? dashMessageAddress : bip322Address);
            await page.locator('#verify-message').fill(profile.id === 'dash-community' ? dashMessage : bip322Message);
            await page
              .locator('#verify-signature')
              .fill(profile.id === 'dash-community' ? dashMessageSignature : bip322Signature);
            ...
            if (profile.id === 'multi-chain') {
              ...
              assert.match(
                await page.locator('#script-wrappers').innerText(),
                /Compiled descriptor data[\s\S]*P2WSH[\s\S]*bc1q/u,
              );
            }
            ...
            assert.match(
              await page.locator('#policy-address').innerText(),
              profile.id === 'dash-community' ? /^7/u : /^bc1q/u,
            );
            ...
            if (profile.id === 'dash-community') {
              ...
              assert.equal(
                await page.locator('#builder-error').isHidden(),
                true,
                await page.locator('#builder-error').innerText(),
              );
              await page.locator('#policy-results').waitFor({ state: 'visible' });
            ...
            run.checks.push(
              'Offline PSBT summary/advanced decoding, signed-message verification, script decoding, P2SH/P2WSH/P2TR/MuSig2 policy build and no HTTP',
            );
          }
          run.layouts = [];
          for (const width of [1280, 390]) {
            ...
            await page.screenshot({
              path: resolve(output, `${browserName}-${profile.id}-${toolId}-${width}.png`),
              fullPage: true,
            });
            ...
            assert.ok(
              dimensions.width <= dimensions.viewport + 2,
              `Horizontal overflow: ${dimensions.width}px document / ${dimensions.viewport}px viewport`,
            );
            assert.ok(
              dimensions.height - dimensions.footerBottom < 200,
              `Excess blank space below footer: ${Math.round(dimensions.height - dimensions.footerBottom)}px`,
            );
            if (toolId === 'discovery-scanner') {
              const frameHeight = await page
                .locator('#recovery-secret-vault')
                .evaluate((element) => element.getBoundingClientRect().height);
              run.layouts.at(-1).frameHeight = frameHeight;
              assert.ok(
                Math.abs(frameHeight - dimensions.footerBottom) < 200,
                'Vault frame height does not follow content',
              );
            }
          }
          ...
          assert.ok(
            !run.console.some((message) => /Unsafe attempt to load URL|unique security origins/u.test(message.text)),
            'File-origin navigation error',
          );
          ...
          await page
            .screenshot({ path: resolve(output, `${browserName}-${profile.id}-${toolId}-failure.png`), fullPage: true })
            .catch(() => {});
        } finally {
          await context.close();
          save();
