      ...
      for (const toolId of ['key-derivation', 'activity-viewer', 'discovery-scanner']) {
        ...
        try {
          ...
          if (toolId === 'discovery-scanner') {
            ...
            assert.equal(await scope.locator('#scan-account-range').isChecked(), false);
            assert.equal(await scope.locator('#account-range-details').isVisible(), false);
            await scope.locator('#scan-account-range').check();
            await scope.locator('#recovery-account').fill('0');
            await scope.locator('#account-range-end').fill('2');
            assert.equal(await scope.locator('#account-range-details').isVisible(), true);
            assert.equal(await scope.locator('label[for="recovery-account"]').innerText(), 'First account');
            await scope.locator('#public-source-tab').click();
            assert.equal(await scope.locator('#account-range-panel').isVisible(), false);
            assert.equal(await scope.locator('#account-range-end').isDisabled(), true);
            await scope.locator('#seed-source-tab').click();
            assert.equal(await scope.locator('#scan-account-range').isChecked(), true);
            await scope.locator('#scan-account-range').uncheck();
            assert.equal(await scope.locator('label[for="recovery-account"]').innerText(), 'Account');
            await scope.locator('#public-source-tab').click();
            run.checks.push('Optional account range, inclusive bound, source isolation and single-account restoration');
