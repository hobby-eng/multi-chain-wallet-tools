...
const manifest = resolve(root, 'packages/recovery-sskr-wasm/rust/Cargo.toml');
const compiled = resolve(
  root,
  'packages/recovery-sskr-wasm/rust/target/wasm32-unknown-unknown/release/recovery_sskr_wasm.wasm',
);
const generated = resolve(root, 'packages/recovery-sskr-wasm/generated');
...
try {
  ...
  assertCanonicalWasmBindgenProducer(readFileSync(stagedWasm), 'Sskr WASM');
  const gluePath = resolve(staging, 'recovery_sskr_wasm.js');
  const fullGlue = readFileSync(gluePath, 'utf8');
  function removeSection(source, startMarker, endMarker) {
    const start = source.indexOf(startMarker);
    const end = source.indexOf(endMarker, start + startMarker.length);
    if (start < 0 || end < 0) throw new Error(`Generated Sskr glue is missing ${startMarker.trim()}.`);
  ...
  if (/\bfetch\s*\(|import\.meta|__wbg_load|\b__wbg_init\b/u.test(normalized)) {
    throw new Error('Failed to reduce Sskr wasm-bindgen glue to its synchronous offline API.');
  }
  mkdirSync(generated, { recursive: true });
  copyFileSync(stagedWasm, resolve(generated, 'recovery_sskr_wasm_bg.wasm'));
  await writeOfflineWasmDeclarations(staging, generated, 'recovery_sskr_wasm', 'SSKR WASM');
  writeFileSync(resolve(generated, 'recovery_sskr_wasm.js'), normalized);
  console.log('Generated pinned Blockchain Commons sskr browser WASM.');
} finally {
