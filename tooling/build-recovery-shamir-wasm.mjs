...
const manifest = resolve(root, 'packages/recovery-shamir-wasm/rust/Cargo.toml');
const compiled = resolve(
  root,
  'packages/recovery-shamir-wasm/rust/target/wasm32-unknown-unknown/release/recovery_shamir_wasm.wasm',
);
const generated = resolve(root, 'packages/recovery-shamir-wasm/generated');
...
try {
  ...
  const stagedWasm = resolve(staging, 'recovery_shamir_wasm_bg.wasm');
  assertCanonicalWasmBindgenProducer(readFileSync(stagedWasm), 'Shamir WASM');
  const gluePath = resolve(staging, 'recovery_shamir_wasm.js');
  const fullGlue = readFileSync(gluePath, 'utf8');
  function removeSection(source, startMarker, endMarker) {
    const start = source.indexOf(startMarker);
    const end = source.indexOf(endMarker, start + startMarker.length);
    if (start < 0 || end < 0) throw new Error(`Generated Shamir glue is missing ${startMarker.trim()}.`);
  ...
  if (/\bfetch\s*\(|import\.meta|__wbg_load|\b__wbg_init\b/u.test(normalized)) {
    throw new Error('Failed to reduce Shamir wasm-bindgen glue to its synchronous offline API.');
  }
