...
describe('page-number explorer histories', () => {
  for (const kind of ['core', 'platform', 'identity'] as const) {
    for (const [historyLimit, fault] of [
      [150, 'none'],
      [200, 'none'],
      [150, 'short'],
      [150, 'changed'],
      ...
      it(`${kind}: ${historyLimit}-row display limit, ${fault} page`, async () => {
        ...
        const fetcher = async (input: string): Promise<Response> => {
          ...
          if (url.pathname.endsWith('/status'))
            value =
              kind === 'core'
                ...
                : {
                    ...
                    api: { block: { height: 100, timestamp } },
                  };
            ...
            value = {
              resultSet:
                fault === 'empty-unknown' || (page === 2 && fault === 'short')
                  ? []
                  : rows.slice(
                      (page - 1) * limit - (page === 2 && fault === 'overlap' ? 1 : 0),
                      fault === 'overlap' && page === 2 ? 149 : page * limit,
                    ),
              pagination: {
                total: fault === 'empty-unknown' ? null : rows.length + (page === 2 && fault === 'changed' ? 1 : 0),
              },
            };
        ...
        const queryIds = async (): Promise<string[]> =>
          kind === 'core'
            ? (await queryCoreAddress(core, 'mainnet', historyLimit, undefined, fetcher)).transactions.map(
                (tx) => tx.txid,
              )
            : kind === 'platform'
              ? (
                  await queryPlatformAddressHistory(platform, 'mainnet', historyLimit, undefined, fetcher)
                ).transitions.map((tx) => tx.hash)
              : (
                  await queryPlatformIdentityHistory(identity, 'mainnet', historyLimit, undefined, fetcher)
                ).activity.map((tx) => tx.transactionHash);
        if (fault !== 'none') {
          await expect(queryIds()).rejects.toThrow(
            fault === 'short' || fault === 'empty-unknown'
              ? /ended before/u
              : fault === 'overlap'
                ? /repeated/u
                : /changed during/u,
...
describe('Audit regressions: complete Dash explorer pages', () => {
  ...
  it.each(['resource', 'missing-total', 'oversized', 'malformed-tail', 'pending'])(
    '%s response boundary',
    async (fault) => {
      const calls: number[] = [];
      const fetcher = async (input: string): Promise<Response> => {
        ...
        if (url.pathname.endsWith('/status')) value = { status: 'ok' };
          ...
          value = {
            address: fault === 'resource' ? 'wrong-address' : core,
            txCount: 150,
            balance: '0',
            received: '0',
            sent: '0',
      ...
      if (fault !== 'pending') {
        await expect(queryCoreAddress(core, 'mainnet', 120, undefined, fetcher)).rejects.toThrow();
        return;
      }
      const result = await queryCoreAddress(core, 'mainnet', 200, undefined, fetcher);
      expect(result.transactions.map((row) => row.txid)).toEqual([
        hash(999),
        ...Array.from({ length: 150 }, (_, i) => hash(i)),
      ]);
