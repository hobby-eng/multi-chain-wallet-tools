...
it.each([1, 2048])('reconciles a nearly full tail before a newer %i-action successor', async (successorCount) => {
  ...
  const outcome = await runShieldedPageStream({
    ...
    noteCount: (p) => p.notes.length,
    revision: (p) => p.proofHeight,
    onPage: (p, visit) => {
      applied.push(visit.position);
      const matches =
        visit.position === 0n
          ? [
              incoming(0n, 100n, '11'.repeat(32)),
              ...(p.notes.length === 2048 ? [incoming(2047n, 50n, '22'.repeat(32))] : []),
            ]
          : [];
