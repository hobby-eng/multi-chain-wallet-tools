...
import { assertWatchOnlyBatchInput } from '@ckd/recovery/watch-only.js';
...
vi.mock('@ckd/dash-wasm/dash_shielded_wasm_bg.wasm', async () => {
  ...
  return {
    default: readFileSync(new URL('../../dash-shielded-wasm/generated/dash_shielded_wasm_bg.wasm', import.meta.url)),
  };
});
...
const families = [
  ...(['legacy', 'nested-segwit', 'native-segwit', 'taproot'] as const).map((mode) => ({
    name: `bitcoin-${mode}`,
    derive: (options: Parameters<typeof deriveDashCore>[0]) => deriveBitcoin(mode, options),
    wrapper:
...
describe('account public/private descriptor exports', () => {
  for (const network of ['mainnet', 'testnet'] as const) {
    it.each(families)(
      '$name ' + network + ' exports both branches with account keys matching independent HD derivation',
      (family) => {
        ...
        try {
          ...
          for (const text of [bundle.publicText, bundle.privateText]) {
            ...
            for (const [branch, line] of lines.entries()) {
              ...
              expect(descriptorChecksum(body!)).toBe(sum);
              ...
              for (const index of [0, 37]) {
                ...
                if (child.privateKey !== null)
                  expect(Buffer.from(child.privateKey).toString('hex')).toBe(independent.privateKey.slice(2));
          ...
          for (const line of bundle.scannerText.split('\n')) {
            const detected = family.name.startsWith('bitcoin')
              ? detectBitcoinWatchOnly(line, { auto: false })
              : detectDashWatchOnly(line, { auto: false });
  ...
  it('matches the published Dash Core descriptor checksum vector', () => {
    const descriptor =
      "pkh([c7fe8acb/44'/1'/0']tpubDDgGSmowbmYWepHK5PJYCfzUFrKy1c7PHVumScWELYwwjaGBf73ZD1JD1xc2y4hKQDp4qHUKjxz8HQyJXmM5UQh797enQQSpq8vife8yva8/0/*)";
