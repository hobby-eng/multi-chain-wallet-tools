...
describe('Dash legacy mobile', () => {
  it.each([0, 1, 7])('matches independent DashHD receive/change derivation for account %i', async (account) => {
    const seed = mnemonicToSeed(TEST_MNEMONIC);
    const referenceRoot = await DashHd.fromSeed(seed, { purpose: 44, coinType: 5 });
    try {
      for (const branch of [0, 1]) {
        const result = deriveDashLegacyMobile({ seed, network: 'mainnet', account, branch, start: 0, count: 1 });
        const path = `m/${account}'/${branch}/0`;
        ...
        expect(result.pathTemplate).toBe(`m/${account}'/${branch}/i`);
