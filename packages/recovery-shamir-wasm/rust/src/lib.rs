//! Narrow WebAssembly boundary around the pinned `blahaj` GF(256) implementation.
...
pub fn split_shamir_shares(
    ...
    let result = (|| {
        ...
        let mut seed: [u8; 32] = random_seed
            .as_slice()
            .try_into()
            .map_err(|_| JsValue::from_str("Shamir randomness must contain exactly 32 bytes."))?;
...
pub fn recover_shamir_secret(
    ...
    let result = (|| {
        ...
        {
            return Err(JsValue::from_str(
                "Shamir share bytes have an invalid length.",
            ));
        }
        if serialized.len() / share_length < usize::from(threshold) {
            return Err(JsValue::from_str(
                "Not enough Shamir shares for the selected threshold.",
            ));
        }
        let shares: Result<Vec<Share>, _> = serialized
            .chunks_exact(share_length)
            .map(Share::try_from)
            .collect();
        let shares = shares
            .map_err(|_| JsValue::from_str("A Shamir share has an invalid serialization."))?;
        Sharks(threshold)
            .recover(&shares)
...
mod tests {
    ...
    #[test]
    fn recovers_sharks_0_5_serialization() {
        // Generated by the previously pinned sharks 0.5.0 build from the
        // secret 00..0f and seed 20..3f. This keeps existing CKD cards
        // recoverable after moving share generation to blahaj.
        let legacy = vec![
            0x01, 0x59, 0x9e, 0x84, 0x20, 0xd8, 0xa6, 0x23, 0xee, 0x67, 0x58, 0x33, 0xfc, 0x26,
            0xf0, 0x90, 0x7a, 0x02, 0xb2, 0x22, 0x13, 0x45, 0xa1, 0x5e, 0x4c, 0xc8, 0xd6, 0xab,
            0x78, 0xf8, 0x58, 0xea, 0x2f, 0xe5, 0x03, 0xeb, 0xbd, 0x95, 0x66, 0x7d, 0xfd, 0x69,
            0x21, 0xb9, 0xfa, 0x41, 0x0f, 0x72, 0x17, 0xb1, 0x90,
        ];
        let expected: Vec<u8> = (0..16).collect();
        assert_eq!(recover_shamir_secret(legacy, 17, 2).unwrap(), expected);
    }
}
