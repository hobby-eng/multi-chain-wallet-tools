...
import { createCodex32Shares, recoverCodex32Shares } from '../src/codex32.js';
...
describe('Codex32 BIP93 backup', () => {
  it('decodes every official unshared BIP93 vector, including alternate padding and long Codex32', () => {
    for (const vector of officialVectors.unshared) {
      for (const record of vector.records) {
        expect(Buffer.from(recoverCodex32Shares([record])).toString('hex')).toBe(vector.seed);
      }
    }
  });
  ...
  it('recovers every threshold subset from the official shared BIP93 vectors', () => {
    for (const vector of officialVectors.shared) {
      ...
      for (const shares of combinations(vector.shares, vector.threshold)) {
        expect(Buffer.from(recoverCodex32Shares(shares)).toString('hex')).toBe(vector.seed);
      }
    }
  });
  ...
  it('rejects every official invalid BIP93 record', () => {
    ...
    for (const record of officialVectors.invalid) expect(() => recoverCodex32Shares([record])).toThrow();
  });
  ...
  it('recovers official BIP93 vector two', () => {
    ...
    expect(Buffer.from(recoverCodex32Shares(shares)).toString('hex')).toBe('d1808e096b35b209ca12132b264662a5');
  });
  ...
  it.each([16, 20, 24, 28, 32, 64])('creates and restores %i-byte shared secrets in any share order', (length) => {
    ...
    const backup = createCodex32Shares(seed, 'seed', 3, 5);
    expect(backup.shares).toHaveLength(5);
    expect(recoverCodex32Shares([backup.shares[4]!, backup.shares[0]!, backup.shares[2]!])).toEqual(seed);
  ...
  it('rejects unsupported lengths, identifiers, thresholds, and counts', () => {
    expect(() => createCodex32Shares(new Uint8Array(15), 'seed', 0, 1)).toThrow(/16, 20, 24, 28, 32, or 64/u);
    expect(() => createCodex32Shares(new Uint8Array(16), 'io1l', 0, 1)).toThrow(/identifier/u);
    expect(() => createCodex32Shares(new Uint8Array(16), 'seed', 1, 1)).toThrow(/threshold/u);
    expect(() => createCodex32Shares(new Uint8Array(16), 'seed', 3, 2)).toThrow(/count/u);
    expect(() => createCodex32Shares(new Uint8Array(16), 'seed', 0, 2)).toThrow(/exactly one/u);
    expect(() => recoverCodex32Shares([])).toThrow(/at least one/u);
  });

  it('rejects bad checksums, repeated shares, mixed sets, and too few shares', () => {
    const seed = new Uint8Array(16).fill(9);
    const first = createCodex32Shares(seed, 'seed', 3, 4);
    const second = createCodex32Shares(seed, 'cash', 3, 4);
    expect(() => recoverCodex32Shares(first.shares.slice(0, 2))).toThrow(
      /3 distinct compatible shares are required; 2 were supplied/u,
    );
    expect(() => recoverCodex32Shares([first.shares[0]!, first.shares[0]!, first.shares[1]!])).toThrow(
      /same Codex32 share index/u,
    );
    expect(() => recoverCodex32Shares([first.shares[0]!, first.shares[1]!, second.shares[2]!])).toThrow(
      /different identifiers/u,
    );
    expect(() => recoverCodex32Shares([first.shares[0]!.toUpperCase(), first.shares[1]!, first.shares[2]!])).toThrow(
      /incompatible prefixes/u,
    );
    const damaged = `${first.shares[0]!.slice(0, -1)}${first.shares[0]!.endsWith('q') ? 'p' : 'q'}`;
    expect(() => recoverCodex32Shares([damaged, first.shares[1]!, first.shares[2]!])).toThrow(/invalid checksum/u);
  });
});
