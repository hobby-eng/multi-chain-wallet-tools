...
import { combineSlip39Mnemonics, generateSlip39Mnemonics, parseSlip39Share } from '../src/slip39.js';
...
describe('SLIP-39', () => {
  it('matches every official Trezor recovery vector', () => {
    for (const [description, mnemonics, secretHex] of vectors as [string, string[], string, string][]) {
      if (secretHex.length === 0) {
        expect(() => combineSlip39Mnemonics(mnemonics, 'TREZOR'), description).toThrow();
      } else {
        expect(Buffer.from(combineSlip39Mnemonics(mnemonics, 'TREZOR')).toString('hex'), description).toBe(secretHex);
      }
    }
  });
  ...
  it('creates recoverable extendable and original shares with deterministic entropy', () => {
    for (const extendable of [false, true]) {
      ...
      const groups = generateSlip39Mnemonics(secret, {
        groupThreshold: 2,
        groups: [
      ...
      expect(combineSlip39Mnemonics(selected, 'correct horse')).toEqual(secret);
  ...
  it('round-trips every BIP39 entropy size in current and legacy formats', () => {
    for (const length of [16, 20, 24, 28, 32]) {
      for (const extendable of [false, true]) {
        ...
        expect(combineSlip39Mnemonics([groups[0]![2]!, groups[0]![0]!], 'ASCII symbols !@#$%^&*()')).toEqual(original);
  ...
  it('accepts normalized casing and distinguishes a wrong passphrase without false validation', () => {
    ...
    expect(combineSlip39Mnemonics([share.toUpperCase()], 'Correct')).toEqual(secret);
    expect(combineSlip39Mnemonics([share], 'Wrong')).not.toEqual(secret);
  });
  ...
  it('rejects mixed sets, duplicate insufficiency, invalid passphrases, and modified words', () => {
    ...
    const first = generateSlip39Mnemonics(new Uint8Array(16).fill(1), options)[0]!;
    const second = generateSlip39Mnemonics(new Uint8Array(16).fill(2), options)[0]!;
    expect(() => combineSlip39Mnemonics([first[0]!])).toThrow(/complete/u);
    expect(() => combineSlip39Mnemonics([first[0]!, second[1]!])).toThrow(/different sets/u);
    expect(() => combineSlip39Mnemonics([first[0]!, first[0]!])).toThrow(/complete/u);
    ...
    expect(() => generateSlip39Mnemonics(new Uint8Array(16), { ...options, passphrase: 'кириллица' })).toThrow(
      /ASCII/u,
    );
  });
});
