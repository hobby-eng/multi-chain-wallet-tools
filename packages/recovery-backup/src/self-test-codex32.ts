...
import { createCodex32Backup, recoverCodex32Seed } from './codex32.js';
...
export function runCodex32SelfTest(): CryptoSelfTestReport {
  ...
  try {
    const record = createCodex32Backup(seed, 'leet', 0, 1);
    expectText('official Codex32 encode vector', record.shares[0]!, expected);
    const recovered = recoverCodex32Seed(record.shares);
    const entropyRecord = createCodex32Backup(entropy, 'seed', 0, 1);
    const recoveredEntropy = recoverCodex32Seed(entropyRecord.shares);
    try {
