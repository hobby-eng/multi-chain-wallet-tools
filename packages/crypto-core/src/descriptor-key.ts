...
export function validateDescriptorPublicKey(
  ...
  throw new Error(
    `Invalid public key: expected a valid ${options.allowXOnly === true ? 'x-only or compressed ' : ''}secp256k1 public key or xpub/tpub descriptor key.`,
  );
}
...
export function materializeDescriptorKey(
  ...
  const match =
    /^(?:\[([0-9a-fA-F]{8}(?:\/[0-9]+['hH]?)*)\])?([xt]pub[1-9A-HJ-NP-Za-km-z]+)((?:\/(?:\d+|<\d+;\d+>|\*))*)$/u.exec(
      normalized,
    );
  if (match === null)
    throw new Error(
      'Staged recovery keys must be compressed public keys or public extended keys with numeric, multipath, and wildcard suffixes.',
    );
  const xpub = match[2]!;
  const expectedPrefix = network === 'mainnet' ? 'xpub' : 'tpub';
  if (!xpub.startsWith(expectedPrefix))
    throw new Error(`The selected ${network} network requires ${expectedPrefix} descriptor keys.`);
  const suffix = match[3]!;
  const segments = suffix.length === 0 ? [] : suffix.slice(1).split('/');
  if (segments.filter((segment) => segment.startsWith('<')).length > 1)
    throw new Error('A BIP389 key may contain only one multipath tuple.');
  if (segments.some((segment) => /^<(\d+);\1>$/u.test(segment)))
    throw new Error('Multipath branches must be distinct.');
  const wildcardPosition = segments.indexOf('*');
  if (wildcardPosition !== -1 && wildcardPosition !== segments.length - 1)
    throw new Error('A descriptor wildcard must be the final derivation step.');
  if (segments.filter((segment) => segment === '*').length > 1)
    throw new Error('A descriptor key may contain only one wildcard.');
  const indexes = segments.map((segment) => {
    if (segment === '*') return wildcardIndex;
    const multipath = /^<(\d+);(\d+)>$/u.exec(segment);
    if (
      multipath !== null &&
      multipath.slice(1).some((value) => !Number.isSafeInteger(Number(value)) || Number(value) >= HARDENED_OFFSET)
    )
      throw new Error('Multipath child exceeds the unhardened BIP32 range.');
    ...
    if (!Number.isSafeInteger(index) || index < 0 || index >= HARDENED_OFFSET)
      throw new Error(`Descriptor child step ${selected} is outside the unhardened BIP32 range.`);
    return index;
  });
...
function validateOrigin(value: string): void {
  ...
  for (const step of origin.split('/').slice(1)) {
    ...
    if (!Number.isSafeInteger(index) || index >= HARDENED_OFFSET)
      throw new Error('Descriptor origin step is outside the BIP32 range.');
  }
}
