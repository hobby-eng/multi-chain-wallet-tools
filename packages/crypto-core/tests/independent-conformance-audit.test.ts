...
describe('independent public-vector cryptographic audit', () => {
  ...
  it.each(['', 'TREZOR', 'é', 'e\u0301', ' public \u0000 🔑 ', ' кириллица 中文 \"quotes\" \\ \n\t 🔑 '])(
    'matches OpenSSL PBKDF2 for public passphrase %j',
    (passphrase) => {
  ...
  it('separates parent passphrase, BIP85 index and child passphrase with independent HMAC/PBKDF2', () => {
    for (const parentPassphrase of ['', 'public parent']) {
      ...
      for (const index of [0, 7, 2147483647]) {
        ...
        const entropy = createHmac('sha512', 'bip-entropy-from-k')
  ...
  it.each([16, 32, 64])('matches independent BIP85 hex HMAC at %i bytes', (bytes) => {
    ...
    const expected = createHmac('sha512', 'bip-entropy-from-k')
      .update(Buffer.from(key.slice(2), 'hex'))
      .digest()
      .subarray(0, bytes)
      .toString('hex');
  ...
  it('matches official BIP38 NFC, embedded NUL and astral-Unicode vector', async () => {
    const result = await decryptBip38(
    ...
    expect(referenceBase58Check(Buffer.concat([Buffer.from([0x80]), result.privateKey]))).toBe(
  ...
  it.each(['bitcoin', 'dash'] as const)('matches external ECDSA signing and byte-length framing for %s', (chain) => {
    ...
    for (const message of ['é🔑', 'a'.repeat(252), 'a'.repeat(253), 'a'.repeat(65535), 'a'.repeat(65536)]) {
      ...
      const size =
        payload.length < 253
          ...
          : payload.length <= 65535
            ? Buffer.from([253, payload.length & 255, payload.length >>> 8])
            : Buffer.from([
                254,
                payload.length & 255,
      ...
      const hash = digest(
        'sha256',
        digest('sha256', Buffer.concat([Buffer.from([magic.length]), magic, size, payload])),
      );
      const signature = signer.sign(hash);
      const expected = Buffer.concat([
        Buffer.from([31 + signature.yParity]),
