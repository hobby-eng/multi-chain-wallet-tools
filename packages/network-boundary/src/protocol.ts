...
import type {
  EvmAccountBatchView as BoundaryEvmAccountBatchView,
  RecoveryHistory,
  UtxoAddressView as BoundaryUtxoAddressView,
} from './data-types.js';
...
// These ceilings are part of the reviewed request contract. Provider clients
// and worker validators must use the same values to keep batching bounded.
...
export interface RecoveryNetworkApi {
  ...
  utxoAddresses(
    ...
    addresses: string[],
    signal?: AbortSignal,
  ): Promise<BoundaryUtxoAddressView[]>;
  evmAccounts(
    network: RecoveryNetwork,
    addresses: string[],
    signal?: AbortSignal,
  ): Promise<BoundaryEvmAccountBatchView>;
}
...
export type RecoveryNetworkRequestInput =
  ...
  | {
      ...
      payload: { coin: 'bitcoin' | 'ethereum'; network: RecoveryNetwork; address: string };
    }
